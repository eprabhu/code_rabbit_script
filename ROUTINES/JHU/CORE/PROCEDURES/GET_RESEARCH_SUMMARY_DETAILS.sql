...
SET LS_FY_MONTH_CRITERIA ='7';
SET LS_FY_START_DATE="-07-01";
SET LS_FY_END_DATE="-06-30";
...
IF AV_WIDGET = 'AWARD_BUDGET_AND_EXPENSES' THEN
	...
	CASE WHEN FINAL_EXPIRATION_DATE < utc_timestamp() THEN NULL ELSE FINAL_EXPIRATION_DATE END AS SORT, ACCOUNT_NUMBER
...
ELSEIF AV_WIDGET = 'AWARD_BY_SPONSOR' THEN
	...
	WHERE A1.AWARD_SEQUENCE_STATUS = 'ACTIVE'  AND A1.AWARD_NUMBER LIKE '%-00001%' 
	AND A1.FINAL_EXPIRATION_DATE >= CURDATE()
	AND A1.AWARD_STATUS_CODE = '1' AND (AV_UNIT_NUMBER IS NULL OR A1.LEAD_UNIT_NUMBER = AV_UNIT_NUMBER
	OR CASE WHEN (AV_UNIT_NUMBER IS NOT NULL AND  AV_DESCENT_FLAG = 'Y') THEN A1.LEAD_UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER = AV_UNIT_NUMBER) ELSE '' END))AS TAB GROUP BY IFNULL(SPONSOR_GROUP,SPONSOR_CODE) ORDER BY AMOUNT DESC LIMIT 10)
	UNION
...
ELSEIF AV_WIDGET IN ('AWARD_BY_SPONSOR_TYPE', 'ACTIVE_AWARDS_BY_SPONSOR_TYPE') THEN
	...
	IF AV_WIDGET = 'AWARD_BY_SPONSOR_TYPE' THEN
		SET LS_ORDER_CONDITION = ' ORDER BY COUNT DESC LIMIT 10';
	END IF;

    	IF AV_WIDGET = 'ACTIVE_AWARDS_BY_SPONSOR_TYPE' THEN
		 SET LS_FILTER_CONDITION = ' AND AWARD_NUMBER LIKE ''%-00001%''';
	END IF;
	...
	SET LS_DYN_SQL = CONCAT('
		SELECT SPONSOR_TYPE_CODE, SPONSOR_TYPE, COUNT(1) as COUNT , IFNULL(SUM(ANTICIPATED_TOTAL_AMOUNT), 0)
		FROM (
			SELECT DISTINCT SPONSOR_TYPE_CODE, SPONSOR_TYPE, AWARD_NUMBER, ANTICIPATED_TOTAL_AMOUNT
			FROM award_master_dataset_rt
			WHERE AWARD_SEQUENCE_STATUS = ''ACTIVE''
			AND AWARD_STATUS_CODE = 1
			AND (
				', LS_UNIT_NUMBER_CONDITION, ' = ''NULL'' 
				OR LEAD_UNIT_NUMBER = ', LS_UNIT_NUMBER_CONDITION, '
				OR CASE 
					WHEN (', LS_UNIT_NUMBER_CONDITION, ' IS NOT NULL AND''', AV_DESCENT_FLAG ,'''= ''Y'') 
					THEN LEAD_UNIT_NUMBER IN (
						SELECT CHILD_UNIT_NUMBER 
						FROM UNIT_WITH_CHILDREN 
						WHERE UNIT_NUMBER = ', LS_UNIT_NUMBER_CONDITION, '
					)
					ELSE ''''
				END
			)
			AND BEGIN_DATE BETWEEN CONCAT(', COL3-1, ', ''', LS_FY_START_DATE, ''') AND CONCAT(', COL3, ', ''', LS_FY_END_DATE, ''')
           
	', LS_FILTER_CONDITION, ') AS TAB
		GROUP BY SPONSOR_TYPE_CODE ', LS_ORDER_CONDITION,'');
...
ELSEIF AV_WIDGET = 'RECENT_AWARDS' THEN
	SELECT DISTINCT U.DISPLAY_NAME, T1.TITLE, T1.PI_NAME, T1.GRANT_CALL_TITLE, T1.AWARD_ID FROM AWARD_MASTER_DATASET_RT T1 
...
ELSEIF AV_WIDGET = 'PROPOSAL_BY_SPONSOR_TYPE' THEN
	...
	INNER JOIN (
    SELECT PROPOSAL_ID FROM EPS_PROPOSAL P JOIN ACCESS_TMP A ON P.HOME_UNIT_NUMBER = A.UNIT_NUMBER
	UNION 
...
SELECT PROPOSAL_ID FROM EPS_PROPOSAL_PERSON_ROLES WHERE PERSON_ID = ''',AV_PERSON_ID,''' 
	UNION     
SELECT CAST(S1.MODULE_ITEM_ID AS UNSIGNED) AS PROPOSAL_ID
 FROM WORKFLOW S1  
	...
	) S ON S.PROPOSAL_ID = T1.PROPOSAL_ID
 ...
 GROUP BY T2.SPONSOR_TYPE_CODE, T3.DESCRIPTION ',LS_ORDER_CONDITION,'');
...
ELSEIF AV_WIDGET = 'AWARDED_PROPOSALS_BY_SPONSOR' THEN
	...
	AND (AV_UNIT_NUMBER IS NULL OR T1.HOME_UNIT_NUMBER = AV_UNIT_NUMBER 
    OR CASE WHEN (AV_UNIT_NUMBER IS NOT NULL AND  AV_DESCENT_FLAG = 'Y') THEN T1.HOME_UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER = AV_UNIT_NUMBER) ELSE '' END
    ) GROUP BY T1.SPONSOR_CODE,T2.SPONSOR_NAME  ORDER BY COUNT DESC LIMIT 10;
...
ELSE
   ...
   ELSE 
	...
	SET LS_DYN_SQL = CONCAT('SELECT U.DISPLAY_NAME,
		SUM(CASE WHEN T1.STATUS_CODE in (1,2,3,4,5,6,8) THEN 1 END) AS SUBMITTED,
		SUM(CASE WHEN T1.STATUS_CODE = 40 THEN 1 END) AS SHORTLISTED,
		SUM(CASE WHEN T1.STATUS_CODE = 2 THEN 1 END) AS AWARDED
	FROM PROPOSAL T1 LEFT JOIN UNIT U ON T1.HOME_UNIT_NUMBER = U.UNIT_NUMBER
	WHERE T1.SUBMISSION_DATE BETWEEN CONCAT(''',COL3-1,LS_FY_START_DATE,''') AND CONCAT(''',COL3,LS_FY_END_DATE,''') 
	 AND T1.PROPOSAL_SEQUENCE_STATUS = ''ACTIVE'' AND T1.STATUS_CODE IN (1,2,3,4,5,6,8) ',LS_UNIT_NUMBER_CONDITION,' GROUP BY T1.HOME_UNIT_NUMBER');
...
ELSE
    ...
    END IF;
				...
				WHERE T1.AWARD_SEQUENCE_STATUS = ''ACTIVE'' AND T1.AWARD_NUMBER LIKE ''%-00001%'' AND T1.STATUS_CODE = 1 
   ...
   ELSEIF AV_WIDGET = 'RESEARCH_SUMMARY_TABLE_DATE' THEN
        ...
        SET LS_DYN_SQL = CONCAT(LS_DYN_SQL,'(SELECT ''Approval In Progress Proposals'' AS APPROVAL_INPROGRESS_PROPOSALS, COUNT(DISTINCT T1.PROPOSAL_ID) AS COUNT,SUM(TOTAL_COST) AS TOTAL_AMOUNT FROM EPS_PROPOSAL T1
            ...
            END
             LEFT OUTER JOIN EPS_PROPOSAL_PERSONS T4 ON T1.PROPOSAL_ID = T4.PROPOSAL_ID AND T4.PI_FLAG =''Y''  
            WHERE T1.STATUS_CODE IN(''3'',''12'',''1'') AND T1.DOCUMENT_STATUS_CODE <> ''3''
            AND (',LS_UNIT_NUMBER_CONDITION,' = ''NULL'' OR T1.HOME_UNIT_NUMBER = ',LS_UNIT_NUMBER_CONDITION,LS_HOME_UNIT_CONDITION,')',LS_LIMIT,' 
            AND (T4.PERSON_ID = ''',AV_PERSON_ID,'''
				or(T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP)) 
            or EXISTS(SELECT (1) from  EPS_PROPOSAL_PERSON_ROLES WHERE PERSON_ID = ''',AV_PERSON_ID,''' AND PROPOSAL_ID = T1.PROPOSAL_ID)
	   ...
	   SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,' INNER JOIN EPS_PROPOSAL_TYPE T3 ON T1.TYPE_CODE = T3.TYPE_CODE
        ...
        SET LS_DYN_SQL = CONCAT(LS_DYN_SQL,'(SELECT ''Approval In Progress Proposals'' AS APPROVAL_INPROGRESS_PROPOSALS, COUNT(DISTINCT T1.PROPOSAL_ID) AS COUNT,SUM(T11.TOTAL_COST) AS TOTAL_AMOUNT FROM EPS_PROPOSAL T1
            ...
            (SELECT ''In Progress Proposals'' AS INPROGRESS_PROPOSALS, COUNT(DISTINCT T1.PROPOSAL_ID) AS COUNT,SUM(T11.TOTAL_COST) AS TOTAL_AMOUNT FROM EPS_PROPOSAL T1 
				',JOIN_CONDITION,' WHERE T1.STATUS_CODE IN(3, 12, 9, 1) AND T1.DOCUMENT_STATUS_CODE <> ''3'' 
                AND (',LS_UNIT_NUMBER_CONDITION,' = ''NULL'' OR T1.HOME_UNIT_NUMBER = ',LS_UNIT_NUMBER_CONDITION,LS_HOME_UNIT_CONDITION,')',LS_LIMIT,' 
				AND (EXISTS(SELECT (1) FROM EPS_PROPOSAL_PERSONS  WHERE PERSON_ID = ''',AV_PERSON_ID,''' AND PROPOSAL_ID = T1.PROPOSAL_ID AND PROP_PERSON_ROLE_ID =3) 
				or(T1.HOME_UNIT_NUMBER IN (SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP)) 
				or EXISTS(SELECT (1) from  EPS_PROPOSAL_PERSON_ROLES WHERE PERSON_ID = ''',AV_PERSON_ID,''' AND PROPOSAL_ID = T1.PROPOSAL_ID)
...
ELSEIF AV_WIDGET = 'INSTITUTE_PROPOSAL_BY_LEAD_UNIT' THEN
   ...
   ELSE 
      SET LS_UNIT_NUMBER_CONDITION = CONCAT(' AND (T1.HOME_UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER = ',LS_UNIT_NUMBER_CONDITION,') OR T1.HOME_UNIT_NUMBER = ',LS_UNIT_NUMBER_CONDITION,')');
	END IF;
SET LS_DYN_SQL = CONCAT(' SELECT T2.DISPLAY_NAME AS UNIT_NAME,
	...
	WHERE T1.PROPOSAL_SEQUENCE_STATUS = ''ACTIVE'' AND T1.STATUS_CODE IN(1,8,2,5) 
	AND T1.SUBMISSION_DATE BETWEEN CONCAT(''',COL3-1,LS_FY_START_DATE,''') AND CONCAT(''',COL3,LS_FY_END_DATE,''') '
...
ELSE
    ...
    SET LS_FY_START_DATE = CONCAT(COL3-1,LS_FY_START_DATE);
    SET LS_FY_END_DATE = CONCAT(COL3,LS_FY_END_DATE);
...
FROM PROPOSAL T1
INNER JOIN SPONSOR T2 ON T1.SPONSOR_CODE = T2.SPONSOR_CODE
WHERE T1.PROPOSAL_SEQUENCE_STATUS = 'ACTIVE' AND T1.STATUS_CODE IN(1,8,2,5) and 
T1.SUBMISSION_DATE BETWEEN LS_FY_START_DATE AND LS_FY_END_DATE GROUP BY T2.SPONSOR_CODE;
...
SELECT T2.SPONSOR_TYPE_CODE, T3.DESCRIPTION AS SPONSOR_TYPE,COUNT(1), IFNULL(SUM(T4.TOTAL_COST),0) AS TOTAL_COST FROM PROPOSAL T1 
		...
		WHERE T1.STATUS_CODE = '1' AND T1.PROPOSAL_SEQUENCE_STATUS = 'ACTIVE' AND (AV_UNIT_NUMBER IS NULL OR T1.HOME_UNIT_NUMBER = AV_UNIT_NUMBER 
		OR CASE WHEN (AV_UNIT_NUMBER IS NOT NULL AND  AV_DESCENT_FLAG = 'Y') THEN T1.HOME_UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER = AV_UNIT_NUMBER) ELSE '' END) 
