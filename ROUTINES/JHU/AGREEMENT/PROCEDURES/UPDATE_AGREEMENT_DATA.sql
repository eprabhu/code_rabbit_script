...
BEGIN
    ...
    DECLARE LS_Q1_ANSWER VARCHAR(4000); -- Q1. Does this agreement or award already have an SAP Grant#?
    DECLARE LS_Q2_ANSWER VARCHAR(4000); -- Q2. Does this agreement involves Research Activities?
 DECLARE LS_Q3_ANSWER VARCHAR(4000); -- Q2. Is this Agreement related to a current Agreement?
 DECLARE LS_Q4_ANSWER VARCHAR(4000);
 DECLARE LS_Q5_ANSWER VARCHAR(4000);-- Is there a FIBI Agreement # you would like to modify?																												   
 ...
 DECLARE LS_MOD_TYPE VARCHAR(2000);
    -- QUESTION_NUMBER will different in across questionnaire.
        ...
        IF LI_QUES_ANS_HEADER_ID IS NOT NULL THEN
            ...
            AND T2.QUESTION_NUMBER = 6841; -- Q1. Does this agreement or award already have an SAP Grant#?

             IF LS_Q1_ANSWER <> '' AND LS_Q1_ANSWER IS NOT NULL AND UPPER(LS_Q1_ANSWER) = UPPER('Yes - That is a Modification of Funded Agreement')  THEN

                SELECT TRIM(T1.ANSWER_LOOKUP_CODE) INTO LS_AWARD_NUMBER FROM QUEST_ANSWER T1
                INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                AND T2.QUESTION_NUMBER = 6842; -- Q1 -> yes -> Please search for and choose the relevant Award.
    
    SELECT TRIM(T1.ANSWER) INTO LS_MOD_TYPE FROM QUEST_ANSWER T1
                INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                AND T2.QUESTION_NUMBER = 6906; -- Q1 -> yes -> Please Select Modification Type.
      ...
      INSERT INTO CUSTOM_DATA (
       ...
       SELECT CDE.CUSTOM_DATA_ELEMENTS_ID ,
       '13',
       511,
       AV_AGREEMENT_HEADER_ID,
       0,
       LS_MOD_TYPE,
       LS_MOD_TYPE,
       UTC_TIMESTAMP(),
       ...
       WHERE UPPER(CDE.CUSTOM_ELEMENT_NAME) = UPPER('Modification-Type');
            ...
            IF LS_Q1_ANSWER IS NULL OR LS_Q1_ANSWER = '' OR UPPER(LS_Q1_ANSWER) = 'NO' THEN
                ...
                INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                AND T2.QUESTION_NUMBER = 6843; -- Q1.2 Was a Fibi Proposal Development record (PD) submitted for this agreement/award?

                IF LS_Q2_ANSWER <> '' AND LS_Q2_ANSWER IS NOT NULL AND UPPER(LS_Q2_ANSWER) = UPPER('Yes - Select IPN')  THEN

                    SELECT TRIM(T1.ANSWER_LOOKUP_CODE) INTO LI_PROJECT_ID
                    FROM QUEST_ANSWER T1
                    INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
                    WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                    AND T2.QUESTION_NUMBER = 6844; -- Q1.2 -> yes -> Please search for and choose the relevant IPN.
    ...
    IF LS_Q2_ANSWER <> '' AND LS_Q2_ANSWER IS NOT NULL AND UPPER(LS_Q2_ANSWER) = UPPER('No - You are initiating an Unfunded Agreement. (If this is a Funded Agreement, please submit a Fibi PD and obtain a Fibi IPN before submitting this Agreement)')  THEN
    
     SELECT TRIM(T1.ANSWER) INTO LS_Q3_ANSWER FROM QUEST_ANSWER T1
     INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
     WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
     AND T2.QUESTION_NUMBER = 6845; -- Q1.3  Is this Agreement related to a current Agreement?
     
     
     
     IF LS_Q3_ANSWER <> '' AND LS_Q3_ANSWER IS NOT NULL AND UPPER(LS_Q3_ANSWER) = UPPER(TRIM('No - This is a New Unfunded Agreement')) THEN
       
       SELECT TRIM(T1.ANSWER) INTO LS_Q4_ANSWER FROM QUEST_ANSWER T1
       INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
       WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
                            AND T2.QUESTION_NUMBER IN ( 6911,6913);
                            -- Q1.4  Does this agreement relate to a project funded by a JHU sponsored award/contract??  
 
       IF LS_Q4_ANSWER <> '' AND LS_Q4_ANSWER IS NOT NULL AND UPPER(LS_Q4_ANSWER) = 'YES' THEN
         SELECT TRIM(T1.ANSWER_LOOKUP_CODE) INTO LS_AWARD_NUMBER
         FROM QUEST_ANSWER T1
         INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
         WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID         
                                    AND T2.QUESTION_NUMBER IN ( 6912,6914)
                                    AND T1.ANSWER_LOOKUP_CODE IS NOT NULL; -- Q1.2 -> yes -> Please select the relevant Award
      ...
      IF LS_Q3_ANSWER <> '' AND LS_Q3_ANSWER IS NOT NULL AND UPPER(LS_Q3_ANSWER) = UPPER(TRIM('Yes - This is a Modification of an Unfunded Agreement')) THEN 
	        INSERT INTO CUSTOM_DATA (
       CUSTOM_DATA_ELEMENTS_ID,
       MODULE_ITEM_CODE,
       MODULE_SUB_ITEM_CODE,
       ...
       UTC_TIMESTAMP(),
       AV_UPDATE_USER);
		 SELECT TRIM(T1.ANSWER) INTO LS_Q5_ANSWER FROM QUEST_ANSWER T1
		 INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
		 WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
		 AND T2.QUESTION_NUMBER IN(6907,6909); -- Q1.3  Is there a FIBI Agreement # you would like to modify?	  
	  
	  
			IF UPPER(LS_Q5_ANSWER) = UPPER('YES') THEN 															  
      
         SELECT TRIM(T1.ANSWER_LOOKUP_CODE) INTO LI_PROJECT_ID
         FROM QUEST_ANSWER T1
         INNER JOIN QUEST_QUESTION T2 ON T1.QUESTION_ID = T2.QUESTION_ID
         WHERE T1.QUESTIONNAIRE_ANS_HEADER_ID = LI_QUES_ANS_HEADER_ID
         AND T2.QUESTION_NUMBER IN ( 6908,6910)
                                    AND T1.ANSWER_LOOKUP_CODE IS NOT NULL; -- Q1.2 -> Please select the related agreement
       
         IF LI_PROJECT_ID IS NOT NULL THEN
         
          SELECT AH.AGREEMENT_REQUEST_ID,ASP.SPONSOR_CODE,AH.UNIT_NUMBER INTO  LI_PROJECT_ID,  LS_SPONSOR_CODE, LS_UNIT_NUMBER
          FROM AGREEMENT_HEADER AH
          LEFT JOIN AGREEMENT_SPONSOR ASP ON ASP.AGREEMENT_REQUEST_ID = AH.AGREEMENT_REQUEST_ID 
          WHERE ASP.AGREEMENT_SPONSOR_TYPE_CODE = 1
          AND AH.AGREEMENT_REQUEST_ID = LI_PROJECT_ID;
     
          INSERT INTO ASSOC_AGREEMENT (AGREEMENT_ID, ASSOC_RELATION_TYPE_CODE, ASSOC_CONFIG_CODE, MODULE_CODE, MODULE_ITEM_KEY, MODULE_ITEM_ID, ASSOC_TYPE, UPDATE_USER, UPDATE_TIMESTAMP, IS_IMPORTED) 
          VALUES (AV_AGREEMENT_HEADER_ID, '1', '4', '13', LI_PROJECT_ID , LI_PROJECT_ID,'DIRECT', AV_UPDATE_PERSON_ID, UTC_TIMESTAMP(), 'Y');
                INSERT INTO CUSTOM_DATA (
            CUSTOM_DATA_ELEMENTS_ID,
            MODULE_ITEM_CODE,
            MODULE_SUB_ITEM_CODE,
  ...
  IF LS_AWARD_TYPE_CODE IS NOT NULL AND LS_AWARD_TYPE_CODE = '1' AND IS_DEPT_SPONSOR_NOA > 0 THEN
  ...
  END IF;
            ...
            IF LI_PROJECT_ID IS NOT NULL AND LS_AGREE_TYPE_CODE IS NOT NULL THEN
