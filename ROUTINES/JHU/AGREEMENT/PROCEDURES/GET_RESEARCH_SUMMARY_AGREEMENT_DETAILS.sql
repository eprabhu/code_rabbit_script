...
DECLARE LS_FY_MONTH_CRITERIA VARCHAR(3) DEFAULT '7';
DECLARE LS_FY_START_DATE VARCHAR(10) DEFAULT '-07-01';
DECLARE LS_FY_END_DATE VARCHAR(10) DEFAULT '-06-30';
...
IF AV_STATUS_CODE IS NULL OR AV_STATUS_CODE = '' THEN
	SET LS_STATUS_CONDITION = CONCAT(' AND T1.AGREEMENT_STATUS_CODE IN (''1'',''2'',''3'',''4'',''5'',''6'',''7'',''10'',''11'',''12'',''13'',''14'',''15'',''16'')');
ELSE
...
SET LS_DYN_SQL = CONCAT(LS_DYN_SQL,'
	...
	IF AV_COUNTER = 'P' THEN
			SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' AND T3.AGREEMENT_STATUS_CODE IN (4,12,13,14,15,16)');
		ELSEIF AV_COUNTER = 'A' THEN
			SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' AND T3.AGREEMENT_STATUS_CODE IN (1,2,3,5,7,11)');
		ELSEIF AV_COUNTER = 'U' THEN
			SET LS_DYN_SQL = CONCAT(LS_DYN_SQL, ' AND T3.AGREEMENT_STATUS_CODE IN (6,10)');
	END IF;
