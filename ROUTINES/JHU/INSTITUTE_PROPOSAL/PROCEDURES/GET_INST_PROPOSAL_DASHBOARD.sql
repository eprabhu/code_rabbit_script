CREATE PROCEDURE `GET_INST_PROPOSAL_DASHBOARD`(
...
IF AV_TAB_TYPE = 'ALL_PROPOSALS' THEN
		  ...
		  SET JOIN_CONDITION =  CONCAT(JOIN_CONDITION,' LEFT JOIN PROPOSAL_PERSONS T14 ON T14.PROPOSAL_ID = T1.PROPOSAL_ID AND T14.PROP_PERSON_ROLE_ID = 3');
...
IF AV_TYPE = 'L' THEN  
	SET TAB_QUERY =  CONCAT(' AND T1.STATUS_CODE <> 4',TAB_QUERY);
END IF;
...
END IF;
	SET LS_DYN_SQL =CONCAT('SELECT DISTINCT * FROM(SELECT DISTINCT  T1.PROPOSAL_ID,
											...
											LEFT  OUTER  JOIN PROPOSAL_PERSONS T8 ON T8.PROPOSAL_ID = T1.PROPOSAL_ID AND T8.PROP_PERSON_ROLE_ID = 3 AND T8.PI_FLAG = ''Y'' 
											...
											LEFT JOIN SPONSOR T31 ON T31.SPONSOR_CODE = T1.SPONSOR_CODE ',JOIN_CONDITION,' WHERE T1.PROPOSAL_SEQUENCE_STATUS IN(''ACTIVE'') ',TAB_QUERY,
