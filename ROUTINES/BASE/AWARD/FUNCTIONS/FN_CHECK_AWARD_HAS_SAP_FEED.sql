-- `FN_CHECK_AWARD_HAS_SAP_FEED`; 


CREATE FUNCTION `FN_CHECK_AWARD_HAS_SAP_FEED`(
AV_AWARD_ID	INT) RETURNS varchar(10) 
    DETERMINISTIC
BEGIN
...
DECLARE LS_TABLE_NAME						VARCHAR(30);
		...
		BEGIN
			...
			INSERT INTO SAP_AWARD_FEED_BATCH_ERROR_LOG(FEED_ID,ERROR_MESSAGE, UPDATE_TIMESTAMP, UPDATE_USER)
			VALUES(LI_FEED_ID,CONCAT('IN FN_CHECK_AWARD_HAS_SAP_FEED ',LS_ERROR_MSG),UTC_TIMESTAMP(),LS_UPDATE_USER);
				...
				END IF;
					...
					IF LI_FLAG > 0 THEN
						...
						ELSEIF LS_FEED_STATUS = 'S' THEN
							...
							AND IFNULL(RESPONSIBLE_COST_CENTER,'0') = IFNULL(LS_PROFIT_CENTER,'0')
				...
				BEGIN
					...
					IF LI_FLAG > 0 THEN
						...
						IF LS_FEED_STATUS = 'E' THEN
							...
							FROM SAP_FEED_TMPL_FM_BUDGET T1
							WHERE T1.FUNDED_PROGRAM = LS_IO_CODE
							AND T1.ID = (SELECT MAX(ID) FROM SAP_FEED_TMPL_FM_BUDGET T2 
										 WHERE T1.FUNDED_PROGRAM = T2.FUNDED_PROGRAM
										 )
							AND IFNULL(T1.BCS_VALUE_TYPE,'0') = IFNULL(LS_BCS_VALUE_TYPE,'0')
							AND	IFNULL(T1.PROCESS,'0') = IFNULL(LS_PROCESS,'0')
							AND IFNULL(T1.DOC_TYPE,'0') = IFNULL(LS_FM_DOC_TYPE,'0')
							AND IFNULL(T1.BUDGET_VERSION,'0') = IFNULL(LS_BUDGET_VERSION,'0')
							AND IFNULL(T1.BUDGET_TYPE,'0') = IFNULL(LS_BUDGET_TYPE,'0')
							AND IFNULL(T1.FM_GRANT,'0') = IFNULL(LS_GRANT,'0')
							AND IFNULL(T1.FUND_CODE,'0') = IFNULL(LS_FUND_CODE,'0')
							AND IFNULL(T1.FUND_CENTER,'0') = IFNULL(LS_FUND_CENTER,'0')
							AND IFNULL(T1.COMMITMENT_ITEM,'0') = IFNULL(LS_COMMITMENT_ITEM,'0')
							AND IFNULL(T1.FUNCTIONAL_AREA,'0') = IFNULL(LS_BA_CODE,'0')
							AND IFNULL(T1.BUDGET_AMOUNT,0) = IFNULL(LI_BUDGET_AMOUNT,0)
							AND IFNULL(T1.LOCAL_CURRENCY,'0') = IFNULL(LS_GRANT_CURRENCY,'0')
							AND IFNULL(T1.DISTRIBUTION_KEY,'0') = IFNULL(LS_FM_DISTRIBUTION_KEY,'0')
							AND IFNULL(T1.LINE_ITEM_TEXT,'0') = IFNULL(LS_LINE_ITEM_DESCRIPTION,'0');
