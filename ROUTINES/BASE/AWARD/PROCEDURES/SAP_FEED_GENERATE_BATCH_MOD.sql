CREATE PROCEDURE `SAP_FEED_GENERATE_BATCH_MOD`(AV_FEED_ID VARCHAR(1000))
BEGIN
...
DECLARE LI_PREV_AWARD_LINE_ITEM_COST		DECIMAL(15,2);
DECLARE LS_USER_ACTION						VARCHAR(20);
DECLARE LI_CUR_AWARD_ID						INT(22);
	...
	BEGIN
		...
		BEGIN
			...
			VALUES(LS_ERROR_MSG,NOW(),LS_UPDATE_USER);
		...
		*/
		...
		SELECT COUNT(1) INTO LI_FLAG
		FROM SAP_AWARD_FEED
		WHERE CASE WHEN (AV_FEED_ID) IS NULL THEN FEED_STATUS = 'P' ELSE FEED_ID IN (AV_FEED_ID) END;
		
		IF LI_FLAG > 0 THEN -- Generating Batch Id
		
			INSERT INTO SAP_AWARD_FEED_BATCH(NO_OF_RECORDS, UPDATE_USER, UPDATE_TIMESTAMP)
			VALUES(LI_FLAG,LS_UPDATE_USER,now());
	...
	IF LI_ERR_FLAG = 0 THEN
		...
		BEGIN
			...
			VALUES(LI_BATCH_ID,LI_FEED_ID,LS_ERROR_MSG,NOW(),LS_UPDATE_USER);
		...
		BEGIN
			...
			SELECT 
				FEED_ID,
				AWARD_NUMBER,
				IF(TRIM(USER_ACTION) = '',NULL,TRIM(USER_ACTION))
			FROM SAP_AWARD_FEED 
			...
			IF TRIM(LS_USER_ACTION) = 'REINTERFACE' THEN 
			...
			ELSEIF LS_USER_ACTION IS NULL THEN
				...
				IF LI_PREV_VERSION_NUMBER <> LI_CUR_VERSION_NUMBER THEN
						...
						BEGIN
							...
							*/
							...
							AND T1.SEQUENCE_NUMBER IN (SELECT MAX(T2.SEQUENCE_NUMBER) FROM AWARD_BUDGET_HEADER T2 
													   WHERE T1.AWARD_ID = T2.AWARD_ID AND T2.AWARD_BUDGET_STATUS_CODE = 10);
