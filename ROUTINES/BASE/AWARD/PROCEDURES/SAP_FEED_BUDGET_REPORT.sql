			...
			-- ------------------STARTS FINDING 'FUND CODE'----------------------------------
				...
				BEGIN
					...
					-- ----------------ENDS FINDING CURRENT AND PREVIOUS LINE ITEM COST---------------------------
						...
						SELECT MAX(CUSTOM_DATA_ELEMENTS_ID) INTO LI_CUSTOM_DATA_ELEMENTS_ID
						FROM CUSTOM_DATA T1
						WHERE  T1.MODULE_ITEM_CODE = 1
						AND T1.MODULE_SUB_ITEM_CODE = 0
						AND T1.MODULE_ITEM_KEY = LI_AWARD_ID
						AND T1.MODULE_SUB_ITEM_KEY =0
						AND T1.CUSTOM_DATA_ELEMENTS_ID IN (SELECT CUSTOM_DATA_ELEMENTS_ID FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'GRANT CODE');

				        IF (LI_CUSTOM_DATA_ELEMENTS_ID > 0) THEN
						
								SELECT T1.VALUE INTO LS_GRANT_CODE
								FROM CUSTOM_DATA T1
								WHERE  T1.MODULE_ITEM_CODE = 1
								AND T1.MODULE_SUB_ITEM_CODE = 0
								AND T1.MODULE_ITEM_KEY = LI_AWARD_ID
								AND T1.MODULE_SUB_ITEM_KEY =0
								AND T1.CUSTOM_DATA_ELEMENTS_ID = LI_CUSTOM_DATA_ELEMENTS_ID;
						...
						AND T1.MODULE_ITEM_KEY = LI_AWARD_ID
						AND T1.MODULE_SUB_ITEM_KEY =0
						AND T1.CUSTOM_DATA_ELEMENTS_ID IN (SELECT CUSTOM_DATA_ELEMENTS_ID FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'PROFIT CENTER');	

				        IF (LI_CUSTOM_DATA_ELEMENTS_ID > 0) THEN
						
								SELECT T1.VALUE INTO LS_PROFIT_CENTER
								FROM CUSTOM_DATA T1
								WHERE  T1.MODULE_ITEM_CODE = 1
								AND T1.MODULE_SUB_ITEM_CODE = 0
								AND T1.MODULE_ITEM_KEY = LI_AWARD_ID
