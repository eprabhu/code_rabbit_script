-- `GET_EXT_REVIEW_DASHBOARD`; 

CREATE PROCEDURE `GET_EXT_REVIEW_DASHBOARD`(
...
BEGIN
 ...
 IF LS_ADMIN_RIGHT_COUNT = 0 AND AV_LOGIN_PERSON_ID IS NOT NULL THEN
    ...
    WHERE IFNULL(IS_DELETED, ''N'') <> ''Y'' AND EXT_REVIEWERS_STATUS_CODE NOT IN ("2","3") AND EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 AND EXTERNAL_REVIEWER_ID IN (',AV_LOGIN_PERSON_ID,')');
    SET LS_REVIEWER_JOIN_CONDITION = CONCAT(LS_REVIEWER_JOIN_CONDITION, ' AND IFNULL(T3.IS_DELETED, ''N'') <> ''Y'' AND EXT_REVIEWERS_STATUS_CODE NOT IN ("2","3") AND T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 AND T3.EXTERNAL_REVIEWER_ID IN (', AV_LOGIN_PERSON_ID, ') ');
 ...
 ELSEIF AV_REVIEWER_ID IS NOT NULL AND AV_REVIEWER_ID <> '' THEN
    SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.EXT_REVIEW_ID IN (SELECT EXT_REVIEW_ID FROM EXT_REVIEW_REVIEWERS
    WHERE IFNULL(IS_DELETED, ''N'') <> ''Y'' AND EXTERNAL_REVIEWER_ID IN(',AV_REVIEWER_ID,')) AND ');  
 END IF;                                                        
...
SET LS_OFFSET_CONDITION = CONCAT(' LIMIT ',AV_LIMIT,' OFFSET ',LS_OFFSET);
 ...
 SET LS_QUERY_ONE = 'select distinct * from (select T1.EXT_REVIEW_ID, T1.DESCRIPTION AS REVIEW_TITLE, T2.TITLE AS PROPOSAL_TITLE,
                        GROUP_CONCAT(DISTINCT CASE WHEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 THEN T4.FULL_NAME END SEPARATOR ", ") AS REVIEWERS, T1.EXT_REVIEW_STATUS_CODE, T1.EXT_REVIEW_STATUS_CODE AS REVIEW_STATUS_CODE,
                        T5.DESCRIPTION AS EXT_REVIEW_STATUS, T1.EXT_REVIEW_SERVICE_TYPE_CODE,
                        T6.DESCRIPTION AS EXT_REVIEW_SERVICE_TYPE, T1.UPDATE_TIMESTAMP, MAX(CASE WHEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 THEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE END) AS EXT_REV_REVIEWER_FLOW_STATUS_CODE,
                        MAX(CASE WHEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 THEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE END) AS FLOW_STATUS_CODE,
                        MAX(CASE WHEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 THEN T7.DESCRIPTION END) AS EXT_REV_REVIEWER_FLOW_STATUS,
                        MAX(CASE WHEN T3.EXT_REV_REVIEWER_FLOW_STATUS_CODE <> 6 THEN T3.VERSION END) AS VERSION,
                         CONCAT (T1.MODULE_ITEM_KEY,''-'',lpad(T1.VERSION_NUMBER,3,0)) as EXT_REVIEW_NUMBER from EXT_REVIEW T1
                        inner join EPS_PROPOSAL T2 on T2.PROPOSAL_ID = CAST(T1.MODULE_ITEM_KEY AS UNSIGNED)
                        left join EXT_REVIEW_REVIEWERS T3 on T3.EXT_REVIEW_ID = T1.EXT_REVIEW_ID AND IFNULL(T3.IS_DELETED, ''N'') <> ''Y''  ';
...
SET LS_DYN_SQL =CONCAT(LS_QUERY_ONE, LS_REVIEWER_JOIN_CONDITION, LS_QUERY_TWO, LS_REVIEW_STATUS_EXCLUSION, LS_QUERY_THREE, LS_FILTER_CONDITION,' ',AV_SORT_TYPE,' ',LS_OFFSET_CONDITION);
