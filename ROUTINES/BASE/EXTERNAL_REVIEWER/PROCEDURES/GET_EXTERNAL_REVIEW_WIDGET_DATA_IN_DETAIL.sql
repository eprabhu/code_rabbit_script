...
BEGIN
    ...
    IF AV_WIDGET IN ('PENDING_REVIEW_SUMMARY_BY_REVIEWERS','INPROGRESS_REVIEW_SUMMARY_BY_REVIEWERS','COMPLETED_REVIEW_SUMMARY_BY_REVIEWERS') THEN
        ...
        ELSEIF AV_WIDGET = 'INPROGRESS_REVIEW_SUMMARY_BY_REVIEWERS' THEN
        ...
        IF AV_EXT_REVIEWER_ID <> '' THEN 
            SET LS_EXT_REVIEWER_CONDITION = CONCAT(LS_EXT_REVIEWER_CONDITION,
                ' WHERE T.EXT_REVIEW_ID IN ( SELECT EXT_REVIEW_ID FROM EXT_REVIEW_REVIEWERS ',
                ' WHERE (IS_DELETED = ''N'' OR IS_DELETED IS NULL) ', 
                LS_FILTER_CONDITION, ' AND EXTERNAL_REVIEWER_ID IN(', AV_EXT_REVIEWER_ID, ') ) ');
        END IF;
        ...
        SET LS_DYN_SQL = CONCAT('
            SELECT DISTINCT * FROM (
                ...
                ', LS_JOIN_CONDITION, '
                WHERE T6.EXT_REVIEW_STATUS_CODE NOT IN (1, 8)
                GROUP BY T1.EXT_REVIEW_ID
            ) T ', LS_EXT_REVIEWER_CONDITION, ' ', AV_SORT_TYPE);
    ...
    ELSEIF AV_WIDGET IN ('REVIEW_DRAFT', 'REVIEW_PENDING','REVIEW_INPROGRESS','COMPLETED', 'REVIEW_INACTIVE') THEN
        ...
        SET LS_DYN_SQL = CONCAT('
            SELECT DISTINCT * FROM (
                ...
                LEFT JOIN (
                    SELECT 
                        GROUP_CONCAT(FULL_NAME SEPARATOR ", ") AS REVIEWERS, 
                        EXT_REVIEW_ID 
                    FROM EXT_REVIEW_REVIEWERS E1 
                    LEFT JOIN EXTERNAL_REVIEWER E2 ON E2.EXTERNAL_REVIEWER_ID = E1.EXTERNAL_REVIEWER_ID
                    WHERE (E1.IS_DELETED = ''N'' OR E1.IS_DELETED IS NULL)  
                    GROUP BY EXT_REVIEW_ID
                ) T4 ON T4.EXT_REVIEW_ID = T1.EXT_REVIEW_ID
                ', LS_FILTER_CONDITION, '
            ) T ', AV_SORT_TYPE);
        ...
        SET LS_DYN_SQL = CONCAT('
            SELECT DISTINCT * FROM (
                SELECT 
                    T1.EXT_REVIEW_ID,
                    T1.DESCRIPTION AS REVIEW_TITLE,
                    JSON_ARRAYAGG(JSON_OBJECT(''fullName'', T3.FULL_NAME, ''status'', T6.DESCRIPTION)) AS REVIEWERS,
                ...
                LEFT JOIN EXT_REVIEW_REVIEWERS_STATUS T6 ON T6.EXT_REVIEW_REVIEWERS_STATUS_CODE = T2.EXT_REVIEW_REVIEWERS_STATUS_CODE
                WHERE T2.EXT_REVIEWERS_STATUS_CODE = 1 
                  AND T1.EXT_REVIEW_STATUS_CODE NOT IN (4,7,8) 
                  AND T1.EXT_REVIEW_SERVICE_TYPE_CODE = 2
        ...
        SET LS_DYN_SQL = CONCAT('
            SELECT DISTINCT * FROM (
                SELECT 
                    T1.EXT_REVIEW_ID,
                    T1.DESCRIPTION AS REVIEW_TITLE,
                    JSON_ARRAYAGG(JSON_OBJECT(''fullName'', T3.FULL_NAME, ''status'', T7.DESCRIPTION)) AS REVIEWERS,
                    T5.DESCRIPTION AS REVIEW_STATUS,
                    T1.REQUEST_DATE AS START_DATE,
                    T1.DEADLINE_DATE,
                    CONCAT(MODULE_ITEM_KEY, ''-'', LPAD(VERSION_NUMBER,3,0)) AS EXT_REVIEW_NUMBER
                ...
                LEFT JOIN EXT_REVIEW_SERVICE_TYPE T4 ON T4.EXT_REVIEW_SERVICE_TYPE_CODE = T1.EXT_REVIEW_SERVICE_TYPE_CODE
                LEFT JOIN EXT_REVIEW_STATUS T5 ON T5.EXT_REVIEW_STATUS_CODE = T1.EXT_REVIEW_STATUS_CODE
                LEFT JOIN EXT_REVIEW_REVIEWERS_STATUS T7 ON T7.EXT_REVIEW_REVIEWERS_STATUS_CODE = T2.EXT_REVIEW_REVIEWERS_STATUS_CODE
                WHERE T1.EXT_REVIEW_SERVICE_TYPE_CODE = 1 
    ...
    ELSEIF AV_WIDGET = 'ASSIGNMENT_LIMIT_WIDGET' THEN
        ...
        IF AV_EXT_REVIEWER_ID <> '' THEN 
            SET LS_EXT_REVIEWER_CONDITION = CONCAT(LS_EXT_REVIEWER_CONDITION,
                ' WHERE T1.EXT_REVIEW_ID IN ( SELECT EXT_REVIEW_ID FROM EXT_REVIEW_REVIEWERS ',
                ' WHERE (IS_DELETED = ''N'' OR IS_DELETED IS NULL) ', 
                ' AND EXT_REV_REVIEWER_FLOW_STATUS_CODE NOT IN(5,6) ',
                ' AND EXTERNAL_REVIEWER_ID IN(', AV_EXT_REVIEWER_ID, ') ', LS_FILTER_CONDITION, ') ');
        END IF;
