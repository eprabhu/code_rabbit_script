-- `GET_GRANTCALL_DASHBOARD`;

CREATE PROCEDURE `GET_GRANTCALL_DASHBOARD`(
...
IF AV_TYPE = 'A' THEN
            ...
            IF AV_TITLE IS NOT NULL AND AV_TITLE <> '' THEN 
			 SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.NAME LIKE ',QUOTE(CONCAT('%',AV_TITLE,'%')),' AND ');
			END IF;           
		   ...
		   IF AV_SPONSOR IS NOT NULL AND AV_SPONSOR <> '' THEN 
				SET LS_FILTER_CONDITION = CONCAT(
					LS_FILTER_CONDITION,
					'T.SPONSOR_NAME  LIKE ', QUOTE(CONCAT('%', AV_SPONSOR, '%')),
					' AND '
		   );
			END IF;
			...
			IF AV_FUNDING_SCHEME IS NOT NULL AND AV_FUNDING_SCHEME <> '' THEN 
			 SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.SCHEME_NAME LIKE ',QUOTE(CONCAT('%',AV_FUNDING_SCHEME,'%')),' AND ');
			END IF;
...
END IF;
IF AV_CALENDAR = TRUE THEN
		SET LS_FILTER_CONDITION = CONCAT(LS_FILTER_CONDITION,' T.GRANT_STATUS_CODE <> 6 AND ');
END IF;
IF AV_UNLIMITED = TRUE OR AV_CALENDAR = TRUE THEN
...
END IF;
			SET LS_DYN_SQL = CONCAT('WITH ACCESS_TMP 
							...
							');
SET LS_DYN_SQL =CONCAT(LS_DYN_SQL,'SELECT DISTINCT * FROM (SELECT T1.GRANT_HEADER_ID,
								...
								T1.CLOSING_DATE, T1.NAME, T6.DISPLAY_NAME, T6.SPONSOR_CODE, T6.ACRONYM, T1.HOME_UNIT_NAME, T1.HOME_UNIT_NUMBER, 
								T1.INTERNAL_SUBMISSION_DEADLINE_DATE, T5.SCHEME_NAME, T1.UPDATE_TIMESTAMP, T1.ABBREVIATION )T  ',LS_FILTER_CONDITION,' ',AV_SORT_TYPE,' ',LS_OFFSET_CONDITION );
