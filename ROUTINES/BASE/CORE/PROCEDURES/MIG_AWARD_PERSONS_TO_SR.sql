CREATE PROCEDURE MIG_AWARD_PERSONS_TO_SR (
...
BEGIN
    ...
    DECLARE SEL_AWARD_PERSONS CURSOR FOR 
     SELECT AWARD_PERSON_ID, DEPARTMENT, FULL_NAME, PERSON_ID, PERSON_ROLE_ID, ROLODEX_ID, PERCENTAGE_OF_EFFORT, PI_FLAG, DESIGNATION, PROJECT_ROLE
     FROM AWARD_PERSONS where AWARD_ID = AV_MODULE_ITEM_ID;
 ...
 SET LS_ERROR_FLAG = 'N';
	...
	SELECT REPORTER_PERSON_ID INTO LS_SR_REPORTER_ID from SR_HEADER where SR_HEADER_ID = AV_SR_HEADER_ID; 
  	...
  	SET LS_PROC_LOC = 'SEL_OLD_SR_PERSONS';
		 ...
		 IF  (select COUNT(*) from sr_persons where SR_HEADER_ID = AV_SR_HEADER_ID  > 0) THEN 	
				DELETE FROM INBOX WHERE MODULE_ITEM_KEY = CAST(AV_SR_HEADER_ID AS CHAR) AND MESSAGE_TYPE_CODE = 146;
         ...
         IF (LS_PERSON_ID is not null) THEN
          ...
          AND ROLE_ID NOT IN(SELECT ROLE_ID FROM MODULE_DERIVED_ROLES WHERE MODULE_CODE = AV_DESTINATION_MODULE_CODE AND AUTO_GRANT_TO_MODULE_CREATOR = 'Y');
			IF (LI_COUNT > 0)
				THEN
					...
					INSERT INTO sr_header_history (ACTION_LOG_ID, UPDATE_TIMESTAMP, UPDATE_USER, SR_HEADER_ID, DELETED_PERMISSION, PERMISSION_ASSIGNED_USER) VALUES (
					LI_INSERT_ID, utc_timestamp(6), AV_UPDATE_USER, AV_SR_HEADER_ID,
					...
					IS_SYSTEM_DERIVED_ROLE = 'Y' and
                    AUTO_GRANT_TO_MODULE_CREATOR != 'Y'),
					LS_FULL_NAME);
					SET LI_COUNT = 0;
			END IF;
  	...
  	SET LS_PROC_LOC = 'SEL_AWARD_PERSONS';
			OPEN SEL_AWARD_PERSONS;
            SEL_AWARD_PERSONS:LOOP
			SET NOT_FOUND  = 0;
            FETCH SEL_AWARD_PERSONS
            INTO LI_AWARD_PERSON_ID, LS_DEPARTMENT, LS_FULL_NAME, LS_PERSON_ID, LI_PERSON_ROLE_ID, LI_ROLODEX_ID, LI_PERCENTAGE_OF_EFFORT, LS_PI_FLAG, LS_DESIGNATION, LS_PROJECT_ROLE;
            IF NOT_FOUND = 1 THEN
            LEAVE SEL_AWARD_PERSONS;
            END IF;
		...
		VALUES (LS_DEPARTMENT, LS_FULL_NAME, LS_PERSON_ID, LI_PERSON_ROLE_ID, LI_ROLODEX_ID, AV_SR_HEADER_ID, LI_PERCENTAGE_OF_EFFORT, LS_PI_FLAG, LS_DESIGNATION, null , LS_PROJECT_ROLE, 'Not Applicable','IMPORTED', utc_timestamp(6), AV_UPDATE_USER);
         ...
         SET LI_INSERT_ID = last_insert_id();
        IF (select count(*)  from award_person_unit where AWARD_PERSON_ID = LI_AWARD_PERSON_ID > 0 ) then

			INSERT INTO sr_person_units (SR_HEADER_ID, SR_PERSON_ID, UNIT_NUMBER, LEAD_UNIT_FLAG, UPDATE_TIMESTAMP, UPDATE_USER)
			SELECT AV_SR_HEADER_ID, LI_INSERT_ID, UNIT_NUMBER, LEAD_UNIT_FLAG, utc_timestamp(6), AV_UPDATE_USER 
			FROM 
			award_person_unit 
		WHERE 
			AWARD_PERSON_ID = LI_AWARD_PERSON_ID;
       END IF;
	...
	IF (LS_PERSON_ID is not null ) then
            ...
            THEN
				...
				SELECT AV_SR_HEADER_ID, LS_PERSON_ID, ROLE_ID, utc_timestamp(6), AV_UPDATE_USER, 'Y' FROM module_derived_roles
				WHERE AUTO_GRANT_TO_PI = 'Y' AND MODULE_CODE = 20    
                AND ((LS_PERSON_ID = LS_SR_REPORTER_ID AND AUTO_GRANT_TO_MODULE_CREATOR != 'Y') OR (LS_PERSON_ID <> LS_SR_REPORTER_ID));
                
                SELECT count(*) INTO LS_INSERT_COUNT FROM module_derived_roles
				WHERE AUTO_GRANT_TO_PI = 'Y' AND MODULE_CODE = 20    
                AND ((LS_PERSON_ID = LS_SR_REPORTER_ID AND AUTO_GRANT_TO_MODULE_CREATOR != 'Y') OR (LS_PERSON_ID <> LS_SR_REPORTER_ID));
				
                IF LS_INSERT_COUNT > 0 THEN
					INSERT INTO sr_header_history (ACTION_LOG_ID, UPDATE_TIMESTAMP, UPDATE_USER, SR_HEADER_ID, ADDED_PERMISSION, PERMISSION_ASSIGNED_USER) VALUES 
					(LI_INSERT_ID, utc_timestamp(6), AV_UPDATE_USER, AV_SR_HEADER_ID,(SELECT GROUP_CONCAT(DERIVED_ROLE_NAME SEPARATOR ', ')  
					FROM module_derived_roles WHERE AUTO_GRANT_TO_PI = 'Y' AND MODULE_CODE = 20 
					AND ((LS_PERSON_ID = LS_SR_REPORTER_ID AND AUTO_GRANT_TO_MODULE_CREATOR != 'Y') OR (LS_PERSON_ID <> LS_SR_REPORTER_ID))),LS_FULL_NAME);
                END IF;
            ...
            THEN
                ...
                IF LS_INSERT_COUNT > 0 THEN
					...
					VALUES (LI_INSERT_ID, utc_timestamp(6), AV_UPDATE_USER, AV_SR_HEADER_ID,
					(SELECT GROUP_CONCAT(DERIVED_ROLE_NAME SEPARATOR ', ')  FROM module_derived_roles WHERE AUTO_GRANT_TO_COI = 'Y' AND MODULE_CODE = 20
                    AND ((LS_PERSON_ID = LS_SR_REPORTER_ID AND AUTO_GRANT_TO_MODULE_CREATOR != 'Y') OR (LS_PERSON_ID <> LS_SR_REPORTER_ID))),LS_FULL_NAME);
                END IF;
			...
			THEN
                ...
                SELECT count(*) INTO LS_INSERT_COUNT
				FROM module_derived_roles
				WHERE AUTO_GRANT_TO_OTHERS = 'Y' AND MODULE_CODE = 20
                AND ((LS_PERSON_ID = LS_SR_REPORTER_ID AND AUTO_GRANT_TO_MODULE_CREATOR != 'Y') OR (LS_PERSON_ID <> LS_SR_REPORTER_ID));
                ...
                IF LS_INSERT_COUNT > 0 THEN
					INSERT INTO sr_header_history (ACTION_LOG_ID, UPDATE_TIMESTAMP, UPDATE_USER, SR_HEADER_ID, ADDED_PERMISSION, PERMISSION_ASSIGNED_USER) 
					VALUES (LI_INSERT_ID, utc_timestamp(6), AV_UPDATE_USER, AV_SR_HEADER_ID,
					(SELECT GROUP_CONCAT(DERIVED_ROLE_NAME SEPARATOR ', ')  FROM module_derived_roles WHERE AUTO_GRANT_TO_OTHERS = 'Y' AND MODULE_CODE = 20
                    AND ((LS_PERSON_ID = LS_SR_REPORTER_ID AND AUTO_GRANT_TO_MODULE_CREATOR != 'Y') OR (LS_PERSON_ID <> LS_SR_REPORTER_ID))),LS_FULL_NAME);
                END IF;
			END IF;
		   ...
		   END IF;
      END IF;  
        END LOOP SEL_AWARD_PERSONS;
		CLOSE SEL_AWARD_PERSONS;  
END
