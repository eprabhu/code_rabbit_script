...
WITH ACCESS_TMP 
    ...
    SELECT T1.SPONSOR_CODE,T2.DISPLAY_NAME AS SPONSOR,COUNT(1) AS COUNT FROM PROPOSAL T1 INNER JOIN SPONSOR T2 ON T2.SPONSOR_CODE = T1.SPONSOR_CODE WHERE T1.STATUS_CODE = 2 AND T1.PROPOSAL_SEQUENCE_STATUS = 'ACTIVE'
	AND (AV_UNIT_NUMBER = 'NULL' OR T1.HOME_UNIT_NUMBER = AV_UNIT_NUMBER OR T1.HOME_UNIT_NUMBER IN (SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER = AV_UNIT_NUMBER))	
