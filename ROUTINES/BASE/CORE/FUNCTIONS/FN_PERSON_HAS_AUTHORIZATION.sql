...
-- GRANT MANAGER CHECKING IN ACTIVE AWARD			
				SELECT COUNT(1) INTO LI_ACTIVE_COUNT
					FROM AWARD T1
					INNER JOIN GRANT_CALL_HEADER T2 ON T1.GRANT_HEADER_ID = T2.GRANT_HEADER_ID
        ...
        AND S2.AWARD_SEQUENCE_STATUS='PENDING';
         ...
         SELECT  COUNT(1) INTO LI_COUNT
					...
					PERSON_ROLES PR JOIN
					(SELECT ROLE_ID,RT.RIGHT_NAME 
					FROM ROLE_RIGHTS RR,RIGHTS RT
					WHERE RR.RIGHT_ID = RT.RIGHT_ID
					AND RT.RIGHT_NAME IN ('MODIFY_ALL_VARIATION_REQUESTS')) RLE ON RLE.ROLE_ID = PR.ROLE_ID
					WHERE PR.PERSON_ID = AV_LOGGED_IN_PERSON_ID
					AND (( PR. DESCEND_FLAG = 'Y' AND EXISTS(SELECT 1
                                        FROM UNIT_WITH_CHILDREN WHERE UNIT_NUMBER = PR.UNIT_NUMBER))OR ( PR. DESCEND_FLAG = 'N' AND PR.UNIT_NUMBER = LS_LEAD_UNIT ));
...
ELSEIF AV_MODULE_CODE = 14 THEN
		...
		SELECT COUNT(1) INTO LI_COUNT  FROM  PERSON_ROLES PR JOIN
										(SELECT ROLE_ID,RT.RIGHT_NAME FROM ROLE_RIGHTS RR, RIGHTS RT
										WHERE RR.RIGHT_ID = RT.RIGHT_ID AND RT.RIGHT_NAME IN ('MODIFY_CLAIMS','VIEW_CLAIMS','CLAIM_PREPARER')) RLE ON  RLE.ROLE_ID = PR.ROLE_ID
										WHERE PR.PERSON_ID IN (SELECT DISTINCT PERSON_ID
										FROM GRANT_CALL_FUNDING_SCHEME_MANAGERS
										WHERE FUNDING_SCHEME_CODE IN (SELECT TS3.FUNDING_SCHEME_CODE FROM GRANT_CALL_HEADER TS1
												INNER JOIN AWARD TS2 ON TS1.GRANT_HEADER_ID = TS2.GRANT_HEADER_ID
...
ELSEIF AV_MODULE_CODE = 16 THEN
		...
		SELECT COUNT(1) INTO LI_COUNT  FROM  PERSON_ROLES PR JOIN 
										(SELECT ROLE_ID,RT.RIGHT_NAME FROM ROLE_RIGHTS RR, RIGHTS RT
										WHERE RR.RIGHT_ID = RT.RIGHT_ID AND RT.RIGHT_NAME IN ('VIEW_PROGRESS_REPORT','MODIFY_PROGRESS_REPORT','CREATE_PROGRESS_REPORT')) RLE ON RLE.ROLE_ID = PR.ROLE_ID
										WHERE PR.PERSON_ID IN (SELECT DISTINCT PERSON_ID
										FROM GRANT_CALL_FUNDING_SCHEME_MANAGERS
										WHERE FUNDING_SCHEME_CODE IN (SELECT TS3.FUNDING_SCHEME_CODE FROM GRANT_CALL_HEADER TS1
												INNER JOIN AWARD TS2 ON TS1.GRANT_HEADER_ID = TS2.GRANT_HEADER_ID
...
ELSEIF AV_MODULE_CODE = 20 THEN
	...
	SELECT COUNT(1) INTO LI_COUNT
						...
						(SELECT ROLE_ID,RT.RIGHT_NAME 
						   FROM ROLE_RIGHTS RR,
								RIGHTS RT
							WHERE RR.RIGHT_ID = RT.RIGHT_ID
							  AND RT.RIGHT_NAME IN ('SERVICE_REQUEST_ADMINISTRATOR','CREATE_SERVICE_REQUEST' ,'MODIFY_SERVICE_REQUEST','VIEW_SERVICE_REQUEST')
						) RLE ON RLE.ROLE_ID = PR.ROLE_ID
						WHERE PR.PERSON_ID = AV_LOGGED_IN_PERSON_ID
						  AND (( PR. DESCEND_FLAG = 'Y' AND EXISTS(SELECT 1
																				 FROM UNIT_WITH_CHILDREN 
	...
	AND (REPORTER_PERSON_ID = AV_LOGGED_IN_PERSON_ID);
		...
		IF LI_MODULE_CODE = 1 THEN
			...
			IF LI_ACTIVE_COUNT = 0 THEN
				-- Not active — fetch the active award id using award no
				SELECT AWARD_ID INTO LI_MODULE_ITEM_ID
				FROM AWARD 
				WHERE AWARD_NUMBER = (
						SELECT AWARD_NUMBER 
						FROM AWARD 
	...
	-- GRANT MANAGER CHECKING IN SR
		SELECT LEAD_UNIT_NUMBER INTO LS_LEAD_UNIT
		 ...
		 IF LI_FLAG > 0 THEN
	        SELECT COUNT(1) INTO LI_FLAG FROM GRANT_CALL_HEADER T1
	        INNER JOIN AWARD T2 ON T1.GRANT_HEADER_ID = T2.GRANT_HEADER_ID
	        WHERE T2.AWARD_ID = LI_MODULE_ITEM_ID
	        AND T1.HOME_UNIT_NUMBER <> (SELECT VALUE FROM PARAMETER WHERE PARAMETER_NAME = 'ROOT_UNIT');
		END IF;
                    ...
                    END IF;
					 ...
					 SELECT COUNT(1) INTO LI_COUNT  FROM  PERSON_ROLES PR,
									...
									WHERE FUNDING_SCHEME_CODE IN (SELECT TS3.FUNDING_SCHEME_CODE FROM GRANT_CALL_HEADER TS1
											INNER JOIN AWARD TS2 ON TS1.GRANT_HEADER_ID = TS2.GRANT_HEADER_ID
											INNER JOIN SPONSOR_FUNDING_SCHEME TS3 ON TS3.FUNDING_SCHEME_ID = TS1.FUNDING_SCHEME_ID
											WHERE TS2.AWARD_ID = LI_MODULE_ITEM_ID)
									AND PERSON_ID = AV_LOGGED_IN_PERSON_ID)
									AND RLE.ROLE_ID = PR.ROLE_ID;
