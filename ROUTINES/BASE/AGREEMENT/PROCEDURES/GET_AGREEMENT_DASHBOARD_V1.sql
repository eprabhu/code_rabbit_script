...
BEGIN
 ...
 -- Declare variables
   ...
   -- Custom data element variables
    ...
    SET SESSION optimizer_search_depth =9;
    ...
    SET @DYNAMIC_CTE = CONCAT('WITH ACCESS_TMP AS (
							...
							WORKFLOW_DATA AS (
									SELECT DISTINCT T9.MODULE_ITEM_ID AS AGREEMENT_REQUEST_ID,
									T10.APPROVER_PERSON_ID AS APPROVER_PERSON_ID
									FROM WORKFLOW T9 
							...
							ASSOC_DETAILS AS(
									...
									AGREEMENT_ID,
									PI_PERSON_ID,
									PI_ROLODEX_PERSON_ID,
									SPONSOR_CODE,
									MODULE_ITEM_KEY,
									MODULE_CODE,
									CAST(START_DATE AS DATE) AS START_DATE,
									CAST(END_DATE AS DATE) AS END_DATE
									FROM AG_ASSOC_DETAIL_V
							)
                        ');
    ...
    SET @NEGO_REVIEW_CTE ='NEGOTIATION_AND_REVIEW AS (
        ...
        SET @SPECIAL_REVIEW_CTE ='AGREEMENT_SPECIAL_REVIEW_JSON AS (
                                     SELECT
                                         CAST(AGREEMENT_REQUEST_ID AS UNSIGNED) AS AGREEMENT_REQUEST_ID,
                                         JSON_ARRAYAGG(
                                             JSON_OBJECT(
                                     ...
                                     WHERE  (PROTOCOL_NUMBER IS NOT NULL OR TRIM(PROTOCOL_NUMBER)<>'''')
                                     GROUP BY AGREEMENT_REQUEST_ID
                                 )';    
...
SET @DYNAMIC_CTE = CONCAT(@DYNAMIC_CTE, COALESCE(FN_PREPARE_DASHBOARD_JOINS(@AV_DISPLAY_LIST,  @AV_FILTER_JSON, @CTE_KEY_MAP, @CTE_MAP, 'CTE'),''));
    ...
    'LEFT JOIN (SELECT AGREEMENT_REQUEST_ID,',
    LS_CUSTOM_DATA_JOIN_SEL,
    ' FROM AGREEMENT_CUSTOM_DATA_RT GROUP BY AGREEMENT_REQUEST_ID) CD 
     ON CD.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID'
);
    ...
    SET @JOIN_QUERY_MAP = JSON_OBJECT(
        ...
        'JOIN_AG_ACTION_LOG', 'LEFT JOIN AGMT_ACTION_LOG AL ON T1.AGREEMENT_REQUEST_ID = AL.AGREEMENT_REQUEST_ID',
...
AND S1.NEGOTIATION_ID = S0.NEGOTIATION_ID AND LOCATION_STATUS_CODE NOT IN (3,4))) OR (EXISTS(SELECT 1 FROM WORKFLOW_DATA WHERE AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND APPROVER_PERSON_ID = ''',AV_PERSON_ID,'''))
...
SET @ALL_PENDING_AGREEMENTS_QUERY = CONCAT(
    IF(IS_UNIT_ADMIN > 0,
        CONCAT('((((EXISTS (SELECT S1.NEGOTIATION_LOCATION_ID FROM NEGOTIATION_LOCATION S1 WHERE S1.NEGOTIATION_ID = S0.NEGOTIATION_ID AND LOCATION_STATUS_CODE NOT IN (3,4))) OR (EXISTS(SELECT 1 FROM WORKFLOW_DATA WHERE AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID)) OR (T1.ADMIN_PERSON_ID IS NOT NULL AND T1.AGREEMENT_STATUS_CODE IN (SELECT S.AGREEMENT_STATUS_CODE FROM AGREEMENT_STATUS S WHERE S.IS_TERMINAL <> ''Y''))) AND T1.AGREEMENT_STATUS_CODE IN (SELECT S.AGREEMENT_STATUS_CODE FROM AGREEMENT_STATUS S WHERE S.IS_TERMINAL <> ''Y'')) AND (T1.UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM ACCESS_TMP_ADMIN))) OR ((EXISTS (SELECT S1.NEGOTIATION_LOCATION_ID FROM NEGOTIATION_LOCATION S1 WHERE S1.ASSIGNEE_PERSON_ID=''',AV_PERSON_ID,''' AND S1.NEGOTIATION_ID = S0.NEGOTIATION_ID AND LOCATION_STATUS_CODE NOT IN (3,4))) OR (EXISTS(SELECT 1 FROM WORKFLOW_DATA WHERE AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND APPROVER_PERSON_ID = ''',AV_PERSON_ID,'''))
		OR (T1.ADMIN_PERSON_ID =''',AV_PERSON_ID,''' AND T1.AGREEMENT_STATUS_CODE IN (SELECT S.AGREEMENT_STATUS_CODE FROM AGREEMENT_STATUS S WHERE S.IS_TERMINAL <> ''Y'')))) AND T1.AGREEMENT_STATUS_CODE IN (SELECT S.AGREEMENT_STATUS_CODE FROM AGREEMENT_STATUS S WHERE S.IS_TERMINAL <> ''Y'')'),
        CONCAT(' OR (((EXISTS (SELECT S1.NEGOTIATION_LOCATION_ID FROM NEGOTIATION_LOCATION S1 WHERE S1.NEGOTIATION_ID = S0.NEGOTIATION_ID AND LOCATION_STATUS_CODE NOT IN (3,4))) OR (EXISTS(SELECT 1 FROM WORKFLOW_DATA WHERE AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID)) OR (T1.ADMIN_PERSON_ID IS NOT NULL AND T1.AGREEMENT_STATUS_CODE IN(2,3,6,5))) AND (T1.UNIT_NUMBER IN(SELECT DISTINCT UNIT_NUMBER FROM VIEW_ACCESS_TMP_ADMIN))) OR ((EXISTS (SELECT S1.NEGOTIATION_LOCATION_ID FROM NEGOTIATION_LOCATION S1 WHERE S1.ASSIGNEE_PERSON_ID=''',AV_PERSON_ID,''' AND S1.NEGOTIATION_ID = S0.NEGOTIATION_ID AND LOCATION_STATUS_CODE NOT IN (3,4))) OR (EXISTS(SELECT 1 FROM WORKFLOW_DATA WHERE AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND APPROVER_PERSON_ID = ''',AV_PERSON_ID,'''))
		OR (T1.ADMIN_PERSON_ID =''',AV_PERSON_ID,''' AND T1.AGREEMENT_STATUS_CODE IN (SELECT S.AGREEMENT_STATUS_CODE FROM AGREEMENT_STATUS S WHERE S.IS_TERMINAL <> ''Y'')))) AND T1.AGREEMENT_STATUS_CODE IN (SELECT S.AGREEMENT_STATUS_CODE FROM AGREEMENT_STATUS S WHERE S.IS_TERMINAL <> ''Y'')')
    )
);
...
END IF;
    ...
    SET @ASSOC_START_DATE_CRT='((ASSOC.AGREEMENT_ID IS NOT NULL
                            AND (
                                (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_START_DATE.fromDate'')) IS NULL AND ASSOC.START_DATE <= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_START_DATE.toDate'')))
                                OR (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_START_DATE.toDate'')) IS NULL AND ASSOC.START_DATE >= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_START_DATE.fromDate'')))
                                OR (ASSOC.START_DATE BETWEEN JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_START_DATE.fromDate'')) AND JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_START_DATE.toDate'')))
                                )
                            ))';
    SET @ASSOC_END_DATE_CRT='((ASSOC.AGREEMENT_ID IS NOT NULL
                        AND (
                            (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_END_DATE.fromDate'')) IS NULL AND ASSOC.END_DATE <= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_END_DATE.toDate'')))
                            OR (JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_END_DATE.toDate'')) IS NULL AND ASSOC.END_DATE >= JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_END_DATE.fromDate'')))
                            OR (ASSOC.END_DATE BETWEEN JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_END_DATE.fromDate'')) AND JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_END_DATE.toDate'')))
                            )
                        ))';
    SET @CRT_LINKED_RECORD_PI_CRT = '(
            ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_RECORD_PI.isEmp'')) = ''true'' AND (ASSOC.AGREEMENT_ID IS NOT NULL AND ASSOC.PI_PERSON_ID IN (SELECT res FROM JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_RECORD_PI.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt) ) )
                OR
            ( JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_LINKED_RECORD_PI.isEmp'')) = ''false'' AND (ASSOC.AGREEMENT_ID IS NOT NULL AND ASSOC.PI_ROLODEX_PERSON_ID IN (SELECT res FROM JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, ''$.CRT_AG_ASSOC_RECORD_PI.values''), ''$[*]'' COLUMNS(res LONGTEXT PATH ''$'')) jt) ) )
        )';
...
SET @CRT_AG_KEYWORDS_VALUE = JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,'$.CRT_AG_KEYWORDS'));
SET @CRT_AG_SPONSOR_TYPE_VALUE = JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,'$.CRT_AG_SPONSOR_TYPE'));
 SET @CRT_CRT_AG_KEY_PERSON_VALUE = JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,'$.CRT_AG_KEY_PERSON'));
 SET @CRITERIA_MAP = JSON_OBJECT(
		'CRT_AG_COMPLIANCE_PENDING', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_COMPLIANCE_PENDING''), JSON_QUOTE(CAST(T1.IS_HOLD AS CHAR))))'),
        ...
        'CRT_AG_REQUESTOR', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_REQUESTOR''), JSON_QUOTE(CAST(T1.REQUESTOR_PERSON_ID AS CHAR))))'),
        'CRT_AG_TYPE', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_TYPE''), JSON_QUOTE(CAST(T1.AGREEMENT_TYPE_CODE AS CHAR))))'),
        'CRT_AG_STATUS', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_STATUS''), JSON_QUOTE(CAST(T1.AGREEMENT_STATUS_CODE AS CHAR))))'),
        'CRT_AG_LEAD_UNIT', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_LEAD_UNIT''), JSON_QUOTE(CAST(T1.UNIT_NUMBER AS CHAR))))'),
		'CRT_AG_SPONSOR', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_SPONSOR''), JSON_QUOTE(CAST(SPO.SPONSOR_CODE AS CHAR))))'),
        'CRT_AG_PI', @CRT_AG_PI_CRT,
        'CRT_AG_REVIEW_STATUS', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_REVIEW_STATUS''), JSON_QUOTE(CAST(T1.REVIEW_STATUS_CODE AS CHAR))))'),
        'CRT_AG_SPONSOR_TYPE', CONCAT('( @CRT_AG_SPONSOR_TYPE_VALUE IS NULL OR EXISTS (SELECT 1 FROM AGREEMENT_SPONSOR T21 WHERE T21.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND T21.SPONSOR_CODE IN (SELECT SP1.SPONSOR_CODE FROM SPONSOR SP1 WHERE SP1.SPONSOR_TYPE_CODE IN (SELECT value FROM JSON_TABLE( @CRT_AG_SPONSOR_TYPE_VALUE, "$[*]" COLUMNS (value VARCHAR(50) PATH "$")) jt))))'),
        'CRT_AG_ADMINISTRATOR', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_ADMINISTRATOR''), JSON_QUOTE(CAST(T1.ADMIN_PERSON_ID AS CHAR))))'),
        'CRT_AG_CATEGORY', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_CATEGORY''), JSON_QUOTE(CAST(T1.CATEGORY_CODE AS CHAR))))'),
        'CRT_AG_LOCATION_STATUS', CONCAT('(EXISTS (SELECT 1 FROM NEGOTIATION_LOCATION S1
            WHERE S1.NEGOTIATION_ID = S0.NEGOTIATION_ID AND (JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_LOCATION_STATUS''), JSON_QUOTE(CAST(S1.LOCATION_STATUS_CODE AS CHAR))))))'),
        'CRT_AG_SUBMISSION_DATE',@CRT_AG_SUBMISSION_DATE_CRT,
        'CRT_AG_LAST_MOD_DATE', @CRT_AG_LAST_MOD_DATE_CRT,
        'CRT_AG_KEYWORDS', CONCAT('(@CRT_AG_KEYWORDS_VALUE IS NULL OR EXISTS(SELECT 1 FROM AGREEMENT_KEYWORDS AK INNER JOIN SCIENCE_KEYWORD SK ON AK.SCIENCE_KEYWORD_CODE = SK.SCIENCE_KEYWORD_CODE WHERE AK.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND SK.DESCRIPTION LIKE CONCAT("%",@CRT_AG_KEYWORDS_VALUE,"%")))'),
        ...
        'CRT_AG_ADMIN_GROUP', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_ADMIN_GROUP''), JSON_QUOTE(CAST(T1.ADMIN_GROUP_ID AS CHAR))))'),
		'CRT_AG_PI_UNIT', CONCAT('(JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_PI_UNIT''), JSON_QUOTE(CAST(P.HOME_UNIT AS CHAR))))'),
        'CRT_AG_REVIEW_DETAILS', CONCAT('(NR.AGREEMENT_REQUEST_ID IS NOT NULL AND LOWER(NR.REVIEW_DETAILS) LIKE CONCAT(''%'', LOWER(JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_REVIEW_DETAILS''))), ''%''))'),
        ...
        'CRT_AG_KEY_PERSON', CONCAT('( @CRT_CRT_AG_KEY_PERSON_VALUE IS NULL OR EXISTS (SELECT 1 FROM AGREEMENT_PEOPLE AP WHERE AP.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND (
			CASE WHEN JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON, "$.CRT_AG_KEY_PERSON.isEmp")) = "true" THEN AP.PERSON_ID ELSE AP.ROLODEX_ID END) 
			IN (SELECT value FROM JSON_TABLE(JSON_EXTRACT(@AV_FILTER_JSON, "$.CRT_AG_KEY_PERSON.values"), "$[*]" COLUMNS (value VARCHAR(50) PATH "$")) jt)))'),
        'CRT_AG_ASSOC_SPONSOR', '(( ASSOC.AGREEMENT_ID IS NOT NULL AND (JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_ASSOC_SPONSOR''), JSON_QUOTE(ASSOC.SPONSOR_CODE)))))', 
        'CRT_AG_ASSOC_START_DATE', @ASSOC_START_DATE_CRT,
        'CRT_AG_ASSOC_END_DATE', @ASSOC_END_DATE_CRT,
        'CRT_AG_ASSOC_RECORD_ID', '(ASSOC.AGREEMENT_ID IS NOT NULL  AND INSTR(ASSOC.MODULE_ITEM_KEY, JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_ASSOC_RECORD_ID''))) > 0)',
        'CRT_AG_ASSOC_RECORD_PI', @CRT_LINKED_RECORD_PI_CRT,
        'CRT_AG_ASSOC_LINKED_MODULE', '((ASSOC.AGREEMENT_ID IS NOT NULL AND (JSON_CONTAINS(JSON_EXTRACT(@AV_FILTER_JSON,''$.CRT_AG_ASSOC_LINKED_MODULE''), JSON_QUOTE(ASSOC.MODULE_CODE)))))'
    );
...
SET LS_DYN_SQL = CONCAT(@DYNAMIC_CTE,
    CASE WHEN AV_IS_COUNT = TRUE 
        THEN ' SELECT TOTAL_COUNT FROM (SELECT COUNT(DISTINCT T1.AGREEMENT_REQUEST_ID) AS TOTAL_COUNT'
    ...
    ' FROM AGREEMENT_HEADER T1
	 ...
	 LEFT JOIN ASSOC_DETAILS ASSOC ON  ASSOC.AGREEMENT_ID= T1.AGREEMENT_REQUEST_ID',
    IFNULL(@JOIN_QUERY, ''),
    CASE 
