...
DECLARE LS_HEADER_ADMIN_ASSIGNED_DATE LONGTEXT;
DECLARE LS_HEADER_CREATE_DATE LONGTEXT;
DECLARE LS_HEADER_SUBMITTED_DATE LONGTEXT;
DECLARE LS_HEADER_CATEGORY LONGTEXT;
DECLARE LS_CUSTOM_DATA_AGG_SELECT LONGTEXT DEFAULT '';
	...
	SET LS_CUSTOM_DATA_SEL = '';
     ...
     SET LS_HEADER_ADMIN_ASSIGNED_DATE = JSON_UNQUOTE(JSON_EXTRACT(LJ_HEADER,'$.ADMIN_ASSIGNED_DATE'));
     SET LS_HEADER_CREATE_DATE = JSON_UNQUOTE(JSON_EXTRACT(LJ_HEADER,'$.CREATED_DATE'));
     SET LS_HEADER_SUBMITTED_DATE = JSON_UNQUOTE(JSON_EXTRACT(LJ_HEADER,'$.SUBMITTED_DATE'));
     SET LS_HEADER_CATEGORY = JSON_UNQUOTE(JSON_EXTRACT(LJ_HEADER,'$.CATEGORY'));
	 END IF;
	...
	CUSTOM_DATA_ELEMENTS_IDS:LOOP
        ...
        SET LS_DYN_CUST_DATA_ELEMENTS = CONCAT(LS_DYN_CUST_DATA_ELEMENTS,',','CD.CUST_ELEMENTS_ID_',LI_CUSTOM_DATA_ELEMENTS_ID);
			...
			IF LI_DATA_TYPE = 4 THEN
			SET LS_CUSTOM_DATA_AGG_SELECT = CONCAT(
				LS_CUSTOM_DATA_AGG_SELECT, 
				'GROUP_CONCAT(DISTINCT CUST_ELEMENTS_ID_', 
				LI_CUSTOM_DATA_ELEMENTS_ID, 
				' SEPARATOR '','') AS CUST_ELEMENTS_ID_', 
				LI_CUSTOM_DATA_ELEMENTS_ID, ','
			);
			ELSE
			SET LS_CUSTOM_DATA_AGG_SELECT = CONCAT(
	...
	IF LS_CUSTOM_DATA_AGG_SELECT <> '' THEN
    SET LS_CUSTOM_DATA_AGG_SELECT = TRIM(TRAILING ',' FROM LS_CUSTOM_DATA_AGG_SELECT);
	END IF;
   ...
   IF LS_DESCEND_FLAG IS NOT NULL AND LS_DESCEND_FLAG <> '' AND LS_WIDGET_UNIT_NUMBER IS NOT NULL AND LS_WIDGET_UNIT_NUMBER <> '' THEN
	...
	IF LJ_HEADER IS NOT NULL THEN
		SET LJ_HEADER_JSON = LJ_CUST_DATA_HEADER_JSON;
	ELSE
...
JSON_OBJECT( 
        ...
        ''DASHBOARD_DETAILS'', JSON_ARRAYAGG(
                JSON_OBJECT(
                    ...
                    ',IF(LS_REVIEW_LOCATIONS IS NULL, "", ",'REVIEW_LOCATIONS',T.REVIEW_LOCATIONS"),'
                    ',IF(LS_HEADER_ADMIN_ASSIGNED_DATE IS NULL, "", ",'ADMIN_ASSIGNED_DATE', UNIX_TIMESTAMP(T.ADMIN_ASSIGN_DATE)*1000"),'
                    ',IF(LS_HEADER_CREATE_DATE IS NULL, "", ",'CREATED_DATE', UNIX_TIMESTAMP(T.CREATE_TIMESTAMP)*1000"),'
                    ',IF(LS_HEADER_SUBMITTED_DATE IS NULL, "", ",'SUBMITTED_DATE', UNIX_TIMESTAMP(T.SUBMITTED_DATE)*1000"),'
                    ',IF(LS_HEADER_CATEGORY IS NULL, "", ",'CATEGORY',T.CATEGORY"),'
                    ');
		...
		SET LS_DYN_SQL = CONCAT(LS_DYN_SQL,'
                                    ...
                                    FROM
                                        ...
                                        ',IF(LS_HEADER_ADMIN_ASSIGNED_DATE IS NULL, "", ",T1.ADMIN_ASSIGN_DATE"),'
                                        ',IF(LS_REVIEW_LOCATIONS IS NULL, "", ",(SELECT GROUP_CONCAT(CONCAT(NLT.DESCRIPTION,' - ',IFNULL(IF(P.PERSON_ID IS NOT NULL, P.FULL_NAME, (SELECT R.FULL_NAME FROM ROLODEX R WHERE R.ROLODEX_ID = NL.ASSIGNEE_PERSON_ID)) ,''), ' (', NLS.DESCRIPTION, ')') SEPARATOR ', ')
											...
											WHERE NA.ASSOCIATED_PROJECT_ID = T1.AGREEMENT_REQUEST_ID) AS REVIEW_LOCATIONS"),'            
                                        ',IF(LS_HEADER_CREATE_DATE IS NULL, "", ",T1.CREATE_TIMESTAMP"),'
                                        ',IF(LS_HEADER_CATEGORY IS NULL, "", ",AC.DESCRIPTION AS CATEGORY"),'
                                        ',IF(LS_HEADER_SUBMITTED_DATE IS NULL, "", ",AAL.SUBMITTED_DATE AS SUBMITTED_DATE"),'
                                        ');    
...
SET LS_DYN_SQL = CONCAT(LS_DYN_SQL,'
									...
									LEFT JOIN (
									SELECT AGREEMENT_REQUEST_ID', IF(LS_CUSTOM_DATA_AGG_SELECT <> '', CONCAT(', ', LS_CUSTOM_DATA_AGG_SELECT), ''), '
									FROM AGREEMENT_CUSTOM_DATA_RT
									GROUP BY AGREEMENT_REQUEST_ID
									) CD ON CD.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID
                                    ...
                                    ',IF(LS_HEADER_CATEGORY IS NULL, "", "LEFT JOIN AGREEMENT_CATEGORY AC ON AC.CATEGORY_CODE = T1.CATEGORY_CODE"),'
                                    ',IF(LS_HEADER_SUBMITTED_DATE IS NULL, "", "LEFT JOIN (SELECT AGREEMENT_REQUEST_ID, MAX(UPDATE_TIMESTAMP) AS SUBMITTED_DATE FROM AGREEMENT_ACTION_LOG WHERE ACTION_TYPE_CODE IN (2, 20) GROUP BY AGREEMENT_REQUEST_ID) AAL ON AAL.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID "),'
