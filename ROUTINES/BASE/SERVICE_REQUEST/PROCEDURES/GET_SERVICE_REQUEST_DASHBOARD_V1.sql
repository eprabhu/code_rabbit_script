...
BEGIN
    ...
    -- Extract filter JSON paths at runtime (like JSON_EXTRACT at runtime of query)
    SET @CRT_SR_ID = AV_FILTER_JSON->>'$.CRT_SR_ID';
    SET @CRT_SR_SUBJECT = AV_FILTER_JSON->>'$.CRT_SR_SUBJECT';
    SET @CRT_SR_TYPE = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_TYPE');
    SET @CRT_SR_DEPARTMENT = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_DEPARTMENT');
    SET @CRT_SR_PRIORITY = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_PRIORITY');
    SET @CRT_SR_STATUS = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_STATUS');
    SET @CRT_SR_TASK_TYPE = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_TASK_TYPE');
    SET @CRT_SR_TASK_STATUS = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_TASK_STATUS');
    SET @CRT_CATEGORY = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_CATEGORY');
    SET @CRT_SR_ADMIN_GROUP = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_ADMIN_GROUP');
    SET @CRT_SR_ASSI_PERSON = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_ASSI_PERSON');
    SET @CRT_SR_REPORTER = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_REPORTER');
    SET @CRT_ASSOC_SR_SPONSOR = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_ASSOC_SR_SPONSOR');
    SET @CRT_ASSOC_SR_LINKED_ID = AV_FILTER_JSON->>'$.CRT_ASSOC_SR_LINKED_ID';
    SET @CRT_SR_ASSOC_LINKED_MODULE = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_ASSOC_LINKED_MODULE');
    SET @CRT_ASSOC_START_DATE_fromDate = AV_FILTER_JSON->>'$.CRT_ASSOC_START_DATE.fromDate';
    SET @CRT_ASSOC_START_DATE_toDate = AV_FILTER_JSON->>'$.CRT_ASSOC_START_DATE.toDate';
    SET @CRT_ASSOC_END_DATE_fromDate = AV_FILTER_JSON->>'$.CRT_ASSOC_END_DATE.fromDate';
    SET @CRT_ASSOC_END_DATE_toDate = AV_FILTER_JSON->>'$.CRT_ASSOC_END_DATE.toDate';
    SET @CRT_SR_CREATE_DATE_fromDate = AV_FILTER_JSON->>'$.CRT_SR_CREATE_DATE.fromDate';
    SET @CRT_SR_CREATE_DATE_toDate = AV_FILTER_JSON->>'$.CRT_SR_CREATE_DATE.toDate';
    SET @CRT_SR_KEY_PERSONS_isEmp = AV_FILTER_JSON->>'$.CRT_SR_KEY_PERSONS.isEmp';
    SET @CRT_SR_KEY_PERSONS_values = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_KEY_PERSONS.values');
    SET @CRT_SR_PI_isEmp = AV_FILTER_JSON->>'$.CRT_SR_PI.isEmp';
    SET @CRT_SR_PI_values = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_PI.values');
    SET @CRT_LINKED_RECORD_PI_isEmp = AV_FILTER_JSON->>'$.CRT_LINKED_RECORD_PI.isEmp';
    SET @CRT_LINKED_RECORD_PI_values = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_LINKED_RECORD_PI.values');
    SET @CRT_SR_CREATED_FROM_EMAIL = JSON_EXTRACT(AV_FILTER_JSON, '$.CRT_SR_CREATED_FROM_EMAIL');
    SET @CRT_SR_CREATED_FROM_EMAIL_VALUE = JSON_UNQUOTE(JSON_EXTRACT(COALESCE(@CRT_SR_CREATED_FROM_EMAIL, '[]'), '$[0]'));
    SET @CRT_SR_FROM_EMAIL_ADDRESS = AV_FILTER_JSON->>'$.CRT_SR_FROM_EMAIL_ADDRESS';

    -- Precompute IN-lists from JSON arrays once (avoids JSON_TABLE per row in main query)
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_TYPE_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_TYPE, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_DEPARTMENT_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_DEPARTMENT, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_PRIORITY_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_PRIORITY, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_STATUS_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_STATUS, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_TASK_TYPE_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_TASK_TYPE, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_TASK_STATUS_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_TASK_STATUS, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_CATEGORY_INLIST FROM JSON_TABLE(COALESCE(@CRT_CATEGORY, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_ASSOC_SR_SPONSOR_INLIST FROM JSON_TABLE(COALESCE(@CRT_ASSOC_SR_SPONSOR, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_ADMIN_GROUP_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_ADMIN_GROUP, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_ASSI_PERSON_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_ASSI_PERSON, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_REPORTER_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_REPORTER, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_ASSOC_LINKED_MODULE_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_ASSOC_LINKED_MODULE, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_KEY_PERSONS_VALUES_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_KEY_PERSONS_values, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_SR_PI_VALUES_INLIST FROM JSON_TABLE(COALESCE(@CRT_SR_PI_values, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;
    SELECT GROUP_CONCAT(QUOTE(res) SEPARATOR ',') INTO @CRT_LINKED_RECORD_PI_VALUES_INLIST FROM JSON_TABLE(COALESCE(@CRT_LINKED_RECORD_PI_values, '[]'), '$[*]' COLUMNS (res LONGTEXT PATH '$')) jt;

    -- Pre-filter assoc: single CTE on sr_assoc_detail_v (index-friendly, no JSON_TABLE in main query)
    SET @ASSOC_FILTER_WHERE = '1=1';
    SET @ASSOC_FILTER_WHERE = CONCAT(@ASSOC_FILTER_WHERE, IF(@CRT_ASSOC_SR_SPONSOR_INLIST IS NOT NULL AND @CRT_ASSOC_SR_SPONSOR_INLIST != '', CONCAT(' AND v.SPONSOR_CODE IN (', @CRT_ASSOC_SR_SPONSOR_INLIST, ')'), ''));
    SET @ASSOC_FILTER_WHERE = CONCAT(@ASSOC_FILTER_WHERE, IF(@CRT_ASSOC_START_DATE_fromDate IS NOT NULL OR @CRT_ASSOC_START_DATE_toDate IS NOT NULL, CONCAT(' AND ((', QUOTE(@CRT_ASSOC_START_DATE_fromDate), ' IS NULL AND CAST(v.START_DATE AS DATE) <= ', QUOTE(@CRT_ASSOC_START_DATE_toDate), ') OR (', QUOTE(@CRT_ASSOC_START_DATE_toDate), ' IS NULL AND CAST(v.START_DATE AS DATE) >= ', QUOTE(@CRT_ASSOC_START_DATE_fromDate), ') OR (CAST(v.START_DATE AS DATE) BETWEEN ', QUOTE(@CRT_ASSOC_START_DATE_fromDate), ' AND ', QUOTE(@CRT_ASSOC_START_DATE_toDate), '))'), ''));
    SET @ASSOC_FILTER_WHERE = CONCAT(@ASSOC_FILTER_WHERE, IF(@CRT_ASSOC_END_DATE_fromDate IS NOT NULL OR @CRT_ASSOC_END_DATE_toDate IS NOT NULL, CONCAT(' AND ((', QUOTE(@CRT_ASSOC_END_DATE_fromDate), ' IS NULL AND CAST(v.END_DATE AS DATE) <= ', QUOTE(@CRT_ASSOC_END_DATE_toDate), ') OR (', QUOTE(@CRT_ASSOC_END_DATE_toDate), ' IS NULL AND CAST(v.END_DATE AS DATE) >= ', QUOTE(@CRT_ASSOC_END_DATE_fromDate), ') OR (CAST(v.END_DATE AS DATE) BETWEEN ', QUOTE(@CRT_ASSOC_END_DATE_fromDate), ' AND ', QUOTE(@CRT_ASSOC_END_DATE_toDate), '))'), ''));
    SET @ASSOC_FILTER_WHERE = CONCAT(@ASSOC_FILTER_WHERE, IF(@CRT_ASSOC_SR_LINKED_ID IS NOT NULL AND @CRT_ASSOC_SR_LINKED_ID != '', CONCAT(' AND v.MODULE_ITEM_KEY LIKE CONCAT(''%'', ', QUOTE(@CRT_ASSOC_SR_LINKED_ID), ', ''%'')'), ''));
    SET @ASSOC_FILTER_WHERE = CONCAT(@ASSOC_FILTER_WHERE, IF(@CRT_LINKED_RECORD_PI_VALUES_INLIST IS NOT NULL AND @CRT_LINKED_RECORD_PI_VALUES_INLIST != '', CONCAT(' AND ((', QUOTE(@CRT_LINKED_RECORD_PI_isEmp), ' = ''true'' AND v.PI_PERSON_ID IN (', @CRT_LINKED_RECORD_PI_VALUES_INLIST, ')) OR (', QUOTE(@CRT_LINKED_RECORD_PI_isEmp), ' = ''false'' AND v.PI_ROLODEX_PERSON_ID IN (', @CRT_LINKED_RECORD_PI_VALUES_INLIST, ')))'), ''));
    SET @ASSOC_FILTER_WHERE = CONCAT(@ASSOC_FILTER_WHERE, IF(@CRT_SR_ASSOC_LINKED_MODULE_INLIST IS NOT NULL AND @CRT_SR_ASSOC_LINKED_MODULE_INLIST != '', CONCAT(' AND v.MODULE_CODE IN (', @CRT_SR_ASSOC_LINKED_MODULE_INLIST, ')'), ''));
    SET @ASSOC_FILTER_CTE = CONCAT('ASSOC_FILTER AS (SELECT DISTINCT v.SR_HEADER_ID FROM sr_assoc_detail_v v WHERE ', @ASSOC_FILTER_WHERE, ')');
    SET @ASSOC_FILTER_CRT = '(T1.SR_HEADER_ID IN (SELECT SR_HEADER_ID FROM ASSOC_FILTER))';
       ...
       SET LS_CUSTOM_DATA_JOIN_SEL = CONCAT(
        ...
        IF LS_CUST_CRT_VALUE IS NOT NULL THEN
            ...
            ELSEIF LI_DATA_TYPE IN (4,6,8,9) THEN
                ...
                ELSE
                    SELECT GROUP_CONCAT(QUOTE(value) SEPARATOR ',') INTO @CUST_CRT_INLIST FROM JSON_TABLE(CAST(LS_CUST_CRT_VALUE AS JSON), '$[*]' COLUMNS (value LONGTEXT PATH '$')) AS t;
					SET CUSTOM_CRT_QUERY = CONCAT('(EXISTS(SELECT 1 FROM SR_CUSTOM_DATA_RT WHERE SR_HEADER_ID = T1.SR_HEADER_ID AND ', LS_CUSTOM_DATA_ELEMENT, ' IN (', COALESCE(@CUST_CRT_INLIST, 'NULL'), ') ))');
                END IF;
            ELSE
				IF LI_DATA_TYPE IN (7, 5) THEN
                 SET LS_CUST_CRT_VALUE = JSON_UNQUOTE(JSON_EXTRACT(CAST(LS_CUST_CRT_VALUE AS JSON), '$[0]'));
                END IF;
...
SET @SR_MAIL_CREATION_CTE = 'SR_MAIL_CREATION AS (
    SELECT SR_HEADER_ID, MAIL_ID, CREATE_TIMESTAMP, CREATE_USER, FROM_EMAIL_ADDRESS
    FROM (
        SELECT smi.SR_HEADER_ID,
               smi.MAIL_ID,
               smi.CREATE_TIMESTAMP,
               smi.CREATE_USER,
               smi.FROM_EMAIL_ADDRESS,
               ROW_NUMBER() OVER (
                   PARTITION BY smi.SR_HEADER_ID
                   ORDER BY (smi.CREATE_TIMESTAMP IS NULL), smi.CREATE_TIMESTAMP ASC, smi.MAIL_ID ASC
               ) AS rn
        FROM sr_mail_informations smi
 ...
 SET @CTE_MAP = JSON_OBJECT(
    ...
    JSON_OBJECT('CTE_ASSOC', JSON_ARRAY('COL_ASSOC_SR_SPONSOR','COL_ASSOC_SR_LINKED_ID','COL_LINKED_RECORD_PI','COL_ASSOC_START_DATE','COL_ASSOC_END_DATE','CRT_ASSOC_SR_SPONSOR','CRT_ASSOC_SR_LINKED_ID','CRT_LINKED_RECORD_PI','CRT_ASSOC_START_DATE','CRT_ASSOC_END_DATE')),
...
SET @DYNAMIC_CTE = CONCAT(@DYNAMIC_CTE, ', ', @ASSOC_FILTER_CTE);
SET @DYNAMIC_CTE = CONCAT(@DYNAMIC_CTE, IF(
    @CRT_SR_CREATED_FROM_EMAIL_VALUE IN ('Y', 'N')
    OR (@CRT_SR_FROM_EMAIL_ADDRESS IS NOT NULL AND @CRT_SR_FROM_EMAIL_ADDRESS != ''),
    CONCAT(', ', @SR_MAIL_CREATION_CTE),
    ''
));
...
-- PREPARE JOIN QUERY: START
	...
	'JOIN_SR_TEMP', 'LEFT JOIN SR_TEMP WF ON WF.SR_HEADER_ID = T1.SR_HEADER_ID',
    ...
    'JOIN_ASSOC_FILTER', 'INNER JOIN ASSOC_FILTER AF ON AF.SR_HEADER_ID = T1.SR_HEADER_ID'	
    );
    ...
    JSON_OBJECT('JOIN_ASSOC_FILTER', JSON_ARRAY('CRT_ASSOC_SR_SPONSOR','CRT_ASSOC_SR_LINKED_ID','CRT_LINKED_RECORD_PI','CRT_ASSOC_START_DATE','CRT_ASSOC_END_DATE'))
    );
...
-- PREPARE TAB QUERY: START
    ...
    SET @MY_PENDING_SERVICE_REQUEST = CONCAT('(((T1.ASSIGNEE_PERSON_ID = ''', AV_PERSON_ID, ''') 
                                                OR  WF.SR_HEADER_ID IS NOT NULL 
    ...
    SET @TAB_QUERY_MAP = JSON_OBJECT(
        ...
        'ALL_PENDING_SERVICE_REQUEST', CONCAT(IFNULL(IF(IS_UNIT_ADMIN > 0, ' (T1.STATUS_CODE IN (''2'',''4'',''15'')) AND  (( (EXISTS(SELECT 1 FROM ACCESS_TMP_SR WHERE UNIT_NUMBER = T1.UNIT_NUMBER) OR EXISTS(SELECT 1 FROM SR_HEADER_IDS WHERE SR_HEADER_ID = T1.SR_HEADER_ID) 
		OR EXISTS(SELECT 1 FROM SR_TEMP WHERE SR_HEADER_ID = T1.SR_HEADER_ID)) )', '(1=0)'),''),' OR (EXISTS (SELECT 1 FROM SR_TASK WHERE TASK_STATUS_CODE IN (1,4,3) AND ASSIGNEE_PERSON_ID = ''', AV_PERSON_ID, ''' AND SR_HEADER_ID = T1.SR_HEADER_ID)
...
-- PREPARE CRITERIA QUERY: START
    ...
    SET @SR_CREATE_DATE_CRT = CONCAT('(
                            (', QUOTE(@CRT_SR_CREATE_DATE_fromDate), ' IS NULL AND CAST(T1.CREATE_TIMESTAMP AS DATE) <= ', QUOTE(@CRT_SR_CREATE_DATE_toDate), ')
                            OR (', QUOTE(@CRT_SR_CREATE_DATE_toDate), ' IS NULL AND CAST(T1.CREATE_TIMESTAMP AS DATE) >= ', QUOTE(@CRT_SR_CREATE_DATE_fromDate), ')
                            OR (CAST(T1.CREATE_TIMESTAMP AS DATE) BETWEEN ', QUOTE(@CRT_SR_CREATE_DATE_fromDate), ' AND ', QUOTE(@CRT_SR_CREATE_DATE_toDate), ')
                            )');
	SET @SR_KEY_PERSON_CRT = CONCAT('(
                                ( ', QUOTE(@CRT_SR_KEY_PERSONS_isEmp), ' = ''true'' AND EXISTS (SELECT 1 FROM SR_PERSONS WHERE SR_HEADER_ID = T1.SR_HEADER_ID AND PERSON_ID IN (', COALESCE(@CRT_SR_KEY_PERSONS_VALUES_INLIST, 'NULL'), ') ) )
                                 OR
                                ( ', QUOTE(@CRT_SR_KEY_PERSONS_isEmp), ' = ''false'' AND EXISTS (SELECT 1 FROM SR_PERSONS WHERE SR_HEADER_ID = T1.SR_HEADER_ID AND ROLODEX_ID IN (', COALESCE(@CRT_SR_KEY_PERSONS_VALUES_INLIST, 'NULL'), ') ) )
                            )');
	SET @CRT_SR_PI_CRT = CONCAT('(
                                ( ', QUOTE(@CRT_SR_PI_isEmp), ' = ''true'' AND EXISTS (SELECT 1 FROM SR_PERSONS WHERE SR_HEADER_ID = T1.SR_HEADER_ID AND PERSON_ROLE_ID = 3 AND PERSON_ID IN (', COALESCE(@CRT_SR_PI_VALUES_INLIST, 'NULL'), ') ) )
                                 OR
                                ( ', QUOTE(@CRT_SR_PI_isEmp), ' = ''false'' AND EXISTS (SELECT 1 FROM SR_PERSONS WHERE SR_HEADER_ID = T1.SR_HEADER_ID AND PERSON_ROLE_ID = 3 AND ROLODEX_ID IN (', COALESCE(@CRT_SR_PI_VALUES_INLIST, 'NULL'), ') ) )
                            )');

    SET @CRT_SR_CREATED_FROM_EMAIL_CRT = '1=1';
    IF @CRT_SR_CREATED_FROM_EMAIL_VALUE = 'Y' THEN
        SET @CRT_SR_CREATED_FROM_EMAIL_CRT = '(EXISTS (SELECT 1 FROM SR_MAIL_CREATION SMC WHERE SMC.SR_HEADER_ID = T1.SR_HEADER_ID))';
    ELSEIF @CRT_SR_CREATED_FROM_EMAIL_VALUE = 'N' THEN
        SET @CRT_SR_CREATED_FROM_EMAIL_CRT = '(NOT EXISTS (SELECT 1 FROM SR_MAIL_CREATION SMC WHERE SMC.SR_HEADER_ID = T1.SR_HEADER_ID))';
    END IF;

        SET @CRITERIA_MAP = JSON_OBJECT(
            'CRT_SR_ID', CONCAT('(T1.SR_HEADER_ID LIKE CONCAT("%",', QUOTE(@CRT_SR_ID), ',"%"))'),
            'CRT_SR_SUBJECT', CONCAT('(T1.SUBJECT LIKE CONCAT("%",', QUOTE(@CRT_SR_SUBJECT), ',"%"))'),
            'CRT_SR_TYPE', CONCAT('(T1.TYPE_CODE IN (', COALESCE(@CRT_SR_TYPE_INLIST, 'NULL'), '))'),
            'CRT_SR_DEPARTMENT', CONCAT('(T1.UNIT_NUMBER IN (', COALESCE(@CRT_SR_DEPARTMENT_INLIST, 'NULL'), '))'),
            'CRT_SR_PRIORITY', CONCAT('(T1.PRIORITY_ID IN (', COALESCE(@CRT_SR_PRIORITY_INLIST, 'NULL'), '))'),
            'CRT_SR_STATUS', CONCAT('(T1.STATUS_CODE IN (', COALESCE(@CRT_SR_STATUS_INLIST, 'NULL'), '))'),
			'CRT_SR_TASK_TYPE', CONCAT('(ST.task_type_code IN (', COALESCE(@CRT_SR_TASK_TYPE_INLIST, 'NULL'), '))'),
			'CRT_SR_TASK_STATUS', CONCAT('(ST.task_status_code IN (', COALESCE(@CRT_SR_TASK_STATUS_INLIST, 'NULL'), '))'),
            'CRT_CATEGORY', CONCAT('(T1.CATEGORY_CODE IN (', COALESCE(@CRT_CATEGORY_INLIST, 'NULL'), '))'),
            'CRT_SR_PI', @CRT_SR_PI_CRT,
            'CRT_SR_KEY_PERSONS', @SR_KEY_PERSON_CRT,
            'CRT_ASSOC_SR_SPONSOR', '1=1',
            'CRT_ASSOC_START_DATE', '1=1',
            'CRT_ASSOC_END_DATE', '1=1',
            'CRT_ASSOC_SR_LINKED_ID', '1=1',
            'CRT_LINKED_RECORD_PI', '1=1',
            'CRT_SR_CREATE_DATE', @SR_CREATE_DATE_CRT,
            'CRT_SR_ADMIN_GROUP', CONCAT('(T1.ADMIN_GROUP_ID IN (', COALESCE(@CRT_SR_ADMIN_GROUP_INLIST, 'NULL'), '))'),
            'CRT_SR_ASSI_PERSON', CONCAT('(T1.ASSIGNEE_PERSON_ID IN (', COALESCE(@CRT_SR_ASSI_PERSON_INLIST, 'NULL'), '))'),
            'CRT_SR_REPORTER', CONCAT('(T1.REPORTER_PERSON_ID IN (', COALESCE(@CRT_SR_REPORTER_INLIST, 'NULL'), '))'),
            'CRT_SR_CREATED_FROM_EMAIL', @CRT_SR_CREATED_FROM_EMAIL_CRT,
            'CRT_SR_FROM_EMAIL_ADDRESS', CONCAT('(EXISTS (SELECT 1 FROM SR_MAIL_CREATION SMC WHERE SMC.SR_HEADER_ID = T1.SR_HEADER_ID AND SMC.FROM_EMAIL_ADDRESS LIKE CONCAT(''%'', ', QUOTE(@CRT_SR_FROM_EMAIL_ADDRESS), ', ''%'')))'),
            'CRT_SR_ASSOC_LINKED_MODULE', '1=1'
        ); 
...
END IF;
IF AV_TAB_TYPE IN ('MY_PENDING_SERVICE_REQUEST','ALL_PENDING_SERVICE_REQUEST') THEN
    SET @JOIN_QUERY = CONCAT(' LEFT JOIN SR_TEMP WF ON WF.SR_HEADER_ID = T1.SR_HEADER_ID ', IFNULL(@JOIN_QUERY,''));
END IF;
    SET LS_DYN_SQL = CONCAT(@DYNAMIC_CTE,
        CASE WHEN AV_IS_COUNT = TRUE 
            THEN ' SELECT TOTAL_COUNT FROM (SELECT COUNT(1) AS TOTAL_COUNT'
