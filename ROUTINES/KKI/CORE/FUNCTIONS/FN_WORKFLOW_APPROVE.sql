...
SELECT T1.SUBJECT, IFNULL(T2.DESCRIPTION, '') INTO LS_TITLE, LS_SERVICE_REQUEST_TYPE
FROM SR_HEADER T1
LEFT JOIN SR_TYPE T2 ON T1.TYPE_CODE = T2.TYPE_CODE
...
ELSEIF AV_MODULE_CODE = 20 THEN
SET LS_USER_MESSAGE = CONCAT('#', LS_MODULE_ITEM_ID, ' | ', IFNULL(LS_SERVICE_REQUEST_TYPE, ''), ' | ', IFNULL(LS_TITLE, ''));    						   
ELSE
	...
	UPDATE INBOX T1 INNER JOIN (SELECT INBOX_ID FROM INBOX  WHERE MODULE_CODE = AV_MODULE_CODE
        ...
        AND TO_PERSON_ID IN(
			SELECT APPROVER_PERSON_ID   FROM WORKFLOW_DETAIL 
                                ...
                                SET T1.OPENED_FLAG = 'Y', UPDATE_TIMESTAMP = UTC_TIMESTAMP(), UPDATE_USER = AV_UPDATE_USER;
