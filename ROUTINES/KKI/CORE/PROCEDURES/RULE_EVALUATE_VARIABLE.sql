...
SELECT T1.LVALUE,
    ...
    SELECT T1.TABLE_NAME,T1.COLUMN_NAME,T1.LOOKUP_WINDOW_NAME  
    INTO LS_TABLE_NAME,LS_COLUMN_NAME,LS_LOOKUP_WINDOW_NAME
	...
	ELSEIF AV_MODULE_CODE = 20 AND AV_SUBMODULE_CODE IN (1,0) THEN 
	...
	ELSEIF AV_MODULE_CODE = 14 AND AV_SUBMODULE_CODE = 0 THEN 
	            IF UPPER(LS_TABLE_NAME) = 'AWARD' THEN 
					SET LS_VARIABLE = CONCAT('SELECT T2.',LS_COLUMN_NAME,' INTO @RESULT FROM CLAIM T1 INNER JOIN AWARD T2 ON T1.AWARD_ID = T2.AWARD_ID WHERE T1.CLAIM_ID = ''',AV_MODULE_ITEM_KEY,'''');
                ELSE IF LS_OPERATOR = '%' THEN
					SET LS_VARIABLE = CONCAT('SELECT COUNT(1) INTO @RESULT FROM CLAIM WHERE CLAIM_ID = ''',AV_MODULE_ITEM_KEY,''' AND ',LS_COLUMN_NAME,' LIKE ''%',LS_RVALUE,'%''');
				ELSE
					SET LS_FILTER_CONDTION = CONCAT(' WHERE CLAIM_ID =',AV_MODULE_ITEM_KEY);
					SET LS_VARIABLE = CONCAT('SELECT ',LS_COLUMN_NAME,' INTO @RESULT FROM ',LS_TABLE_NAME,LS_FILTER_CONDTION);
				END IF;
			END IF;
