CREATE PROCEDURE GET_AWARD_DASHBOARD_DYNAMIC_JSON(
...
-- CUSTOM DATA
  ...
  DECLARE key_prefix VARCHAR(255) DEFAULT '';
...
OR T1.AWARD_ID IN(SELECT AWARD_ID FROM AWARD_PERSON_ROLES WHERE PERSON_ID=''',AV_PERSON_ID,'''  AND ROLE_ID IN (12,13)))

						AND T1.AWARD_SEQUENCE_STATUS = ''ACTIVE''  AND T4.STATUS_CODE NOT IN (11,12,5,4) )
						OR (T1.AWARD_ID IN (SELECT AWARD_ID FROM AWARD_CONTACT WHERE PERSON_ID = ''',AV_PERSON_ID,''' AND CONTACT_TYPE_CODE = 1
						AND T1.AWARD_SEQUENCE_STATUS = ''ACTIVE''  AND T4.STATUS_CODE NOT IN (11,12,5,4)) ) ');
...
IF (AV_SORT_TYPE IS NULL OR AV_SORT_TYPE = '0')  AND AV_TAB_TYPE <> 'ALL_AWARDS' THEN
        SET AV_SORT_TYPE =  CONCAT(' ORDER BY T.UPDATE_TIMESTAMP DESC ');
ELSEIF (AV_SORT_TYPE IS NULL OR AV_SORT_TYPE = '0') AND AV_TAB_TYPE = 'ALL_AWARDS' THEN
...
IF SELECTED_FIELD_LIST IS NULL THEN
SET SELECTED_FIELD_LIST = '';
END IF;

IF JOIN_CONDITION IS NULL THEN
SET JOIN_CONDITION = '';
END IF;

IF LS_FILTER_CONDITION IS NULL THEN
SET LS_FILTER_CONDITION = '';
END IF;
