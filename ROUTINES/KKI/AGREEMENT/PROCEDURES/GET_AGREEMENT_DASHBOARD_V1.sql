...
SET @DYNAMIC_CTE = CONCAT(@DYNAMIC_CTE, COALESCE(FN_PREPARE_DASHBOARD_JOINS(@AV_DISPLAY_LIST,  @AV_FILTER_JSON, @CTE_KEY_MAP, @CTE_MAP, 'CTE'),''));
    ...
    IF AV_IS_COUNT = FALSE THEN
        SET @QUERY_MAP = JSON_OBJECT(
            ...
            'COL_AG_KKI_STATUS', 'CASE WHEN T1.IS_HOLD = ''Y'' THEN CONCAT(T2.DESCRIPTION, '' - '', ''On Hold'') ELSE T2.DESCRIPTION END',
    ...
    SET @CUSTOM_DATA_JOIN = CONCAT('LEFT JOIN (SELECT AGREEMENT_REQUEST_ID,', LS_CUSTOM_DATA_JOIN_SEL, ' FROM AGREEMENT_CUSTOM_DATA_RT) CD ON CD.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID');
...
SET @CRT_AG_SPONSOR_TYPE_VALUE = JSON_UNQUOTE(JSON_EXTRACT(@AV_FILTER_JSON,'$.CRT_AG_SPONSOR_TYPE'));
 ...
 SET @CRITERIA_MAP = JSON_OBJECT(
        ...
        'CRT_AG_SPONSOR_TYPE', CONCAT('( @CRT_AG_SPONSOR_TYPE_VALUE IS NULL OR EXISTS (SELECT 1 FROM AGREEMENT_SPONSOR T21 WHERE T21.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND T21.SPONSOR_CODE IN (SELECT SP1.SPONSOR_CODE FROM SPONSOR SP1 WHERE SP1.SPONSOR_TYPE_CODE IN (@CRT_AG_SPONSOR_TYPE_VALUE))))'),
        ...
        'CRT_AG_KEY_PERSON', CONCAT('( @CRT_CRT_AG_KEY_PERSON_VALUE IS NULL OR EXISTS (SELECT 1 FROM AGREEMENT_PEOPLE AP WHERE AP.AGREEMENT_REQUEST_ID = T1.AGREEMENT_REQUEST_ID AND AP.PERSON_ID IN (@CRT_CRT_AG_KEY_PERSON_VALUE)))'),
