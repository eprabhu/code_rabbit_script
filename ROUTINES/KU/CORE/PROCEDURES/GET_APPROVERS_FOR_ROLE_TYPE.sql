...
ELSEIF AV_ROLE_TYPE = 48 THEN 
 ...
 ELSEIF AV_ROLE_TYPE = 50 THEN 
        ...
        IF AV_MODULE_CODE = 16 THEN
                ...
                INNER JOIN PERSON T2 ON T1.PERSON_ID = T2.PERSON_ID
                WHERE T1.ROLE_ID = 1379 
                AND (( T1. DESCEND_FLAG = 'Y' AND LS_LEAD_UNIT IN 
							(SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN 
                            WHERE UNIT_NUMBER = T1.UNIT_NUMBER
					))OR ( T1. DESCEND_FLAG = 'N' AND T1.UNIT_NUMBER = LS_LEAD_UNIT ))
                AND T2.STATUS = 'A'; 
        END IF;
...
ELSEIF AV_ROLE_TYPE = 63 THEN 
        ...
        IF AV_MODULE_CODE = 16 THEN
                SELECT GROUP_CONCAT(distinct T1.PERSON_ID)
                FROM PERSON_ROLES T1
                INNER JOIN PERSON T2 ON T1.PERSON_ID = T2.PERSON_ID
                WHERE T1.ROLE_ID = 1390 
                AND (( T1. DESCEND_FLAG = 'Y' AND LS_LEAD_UNIT IN 
							(SELECT CHILD_UNIT_NUMBER FROM UNIT_WITH_CHILDREN 
                            WHERE UNIT_NUMBER = T1.UNIT_NUMBER
					))OR ( T1. DESCEND_FLAG = 'N' AND T1.UNIT_NUMBER = LS_LEAD_UNIT ))
...
ELSEIF AV_ROLE_TYPE = 68 THEN 
	...
	IF AV_MODULE_CODE = 3 THEN
		...
		FROM EPS_PROPOSAL_PERSONS DP
		INNER JOIN PERSON P ON DP.PERSON_ID = P.PERSON_ID
		WHERE DP.PROPOSAL_ID = AV_MODULE_ITEM_ID
		AND DP.PROP_PERSON_ROLE_ID = 1 AND P.STATUS = 'A';	
	...
	ELSEIF AV_MODULE_CODE = 1 THEN
		...
		WHERE AP.AWARD_ID = AV_MODULE_ITEM_ID
		AND AP.PERSON_ROLE_ID = 1 AND P.STATUS = 'A';	
