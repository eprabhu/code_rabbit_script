...
MODULE_ITEM_KEY,
MODULE_ITEM_CODE,
MAX(case when CUSTOM_DATA_ELEMENTS_ID =  (SELECT MAX(CUSTOM_DATA_ELEMENTS_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Research') then ifnull(description,value) end) as RESEARCH_CENTER,
MAX(case when CUSTOM_DATA_ELEMENTS_ID =  (SELECT MAX(CUSTOM_DATA_ELEMENTS_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Sponsor announcement Date') then ifnull(description,value) end) as SPONSOR_ANNOUNCEMENT_DATE,
MAX(case when CUSTOM_DATA_ELEMENTS_ID =  (SELECT MAX(CUSTOM_DATA_ELEMENTS_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Date of Initiation') then ifnull(description,value) end) as DATE_OF_INITIATION,
MAX(case when CUSTOM_DATA_ELEMENTS_ID =  (SELECT MAX(CUSTOM_DATA_ELEMENTS_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Proposal Milestones Mandatory Check') then ifnull(description,value) end) as PROPOSAL_MILESTONES_MANDATORY_CHECK,
MAX(case when CUSTOM_DATA_ELEMENTS_ID =  (SELECT MAX(CUSTOM_DATA_ELEMENTS_ID) FROM CUSTOM_DATA_ELEMENTS WHERE CUSTOM_ELEMENT_NAME = 'Proposal Budget Mandatory Check') then ifnull(description,value) end) as PROPOSAL_BUDGET_MANDATORY_CHECK,
UTC_TIMESTAMP()
