CREATE VIEW ag_assoc_detail_v AS
    SELECT 
    ...
    FROM
        ((((SELECT 
            ag.AGREEMENT_ID AS AGREEMENT_ID,
                ag.MODULE_CODE AS MODULE_CODE,
                ag.MODULE_ITEM_KEY AS MODULE_ITEM_KEY,
                ag.MODULE_ITEM_ID AS MODULE_ITEM_ID,
        ...
        FROM
            (((((((((((assoc_agreement ag
        LEFT JOIN award aw ON (((ag.MODULE_CODE = 1)
            AND (aw.AWARD_NUMBER = ag.MODULE_ITEM_KEY)
            AND (ag.MODULE_ITEM_ID = aw.AWARD_ID))))
        LEFT JOIN (SELECT 
        ...
        FROM
            award_persons
        WHERE
            (award_persons.PERSON_ROLE_ID = 3)) awp ON (((ag.MODULE_CODE = 1)
            AND (awp.AWARD_NUMBER = aw.AWARD_NUMBER)
            AND (awp.AWARD_ID = aw.AWARD_ID))))
        LEFT JOIN agreement_header ah ON (((ag.MODULE_CODE = 13)
            AND (ah.AGREEMENT_REQUEST_ID = ag.MODULE_ITEM_KEY)
            AND (ag.MODULE_ITEM_ID = ah.AGREEMENT_REQUEST_ID))))
        LEFT JOIN (SELECT 
        ...
        FROM
            agreement_people
        WHERE
            (agreement_people.PEOPLE_TYPE_ID = 3)) ahp ON (((ag.MODULE_CODE = 13)
            AND (ahp.AGREEMENT_REQUEST_ID = ah.AGREEMENT_REQUEST_ID))))
        LEFT JOIN agreement_sponsor ahs ON (((ag.MODULE_CODE = 13)
            AND (ahs.AGREEMENT_REQUEST_ID = ah.AGREEMENT_REQUEST_ID))))
        LEFT JOIN eps_proposal dev_prop ON (((ag.MODULE_CODE = 3)
            AND (dev_prop.PROPOSAL_ID = ag.MODULE_ITEM_KEY)
            AND (ag.MODULE_ITEM_ID = dev_prop.PROPOSAL_ID))))
        LEFT JOIN (SELECT 
        ...
        WHERE
            (eps_proposal_persons.PROP_PERSON_ROLE_ID = 3)) epp ON (((ag.MODULE_CODE = 3)
            AND (epp.PROPOSAL_ID = dev_prop.PROPOSAL_ID))))
        LEFT JOIN proposal ip ON (((ag.MODULE_CODE = 2)
            AND (ip.PROPOSAL_NUMBER = ag.MODULE_ITEM_KEY)
            AND (ag.MODULE_ITEM_ID = ip.PROPOSAL_ID))))
        LEFT JOIN (SELECT 
        ...
        FROM
            proposal_persons
        WHERE
            (proposal_persons.PROP_PERSON_ROLE_ID = 3)) pp ON (((ag.MODULE_CODE = 2)
            AND (pp.PROPOSAL_ID = ip.PROPOSAL_ID)
            AND (pp.PROPOSAL_NUMBER = ip.PROPOSAL_NUMBER))))
        LEFT JOIN sr_header srh ON (((ag.MODULE_CODE = 20)
            AND (srh.SR_HEADER_ID = ag.MODULE_ITEM_KEY)
            AND (ag.MODULE_ITEM_ID = srh.SR_HEADER_ID))))
        LEFT JOIN (SELECT 
        ...
        FROM
            sr_persons
        WHERE
            (sr_persons.PERSON_ROLE_ID = 3)) srp ON (((ag.MODULE_CODE = 20)
